...
| test_file_handle.py | 文件句柄（读/写/克隆/序列化）+ clone_ref/deep_clone 内部 | 宿主（文件 IO） | ✅ 宿主层保留；clone_ref/deep_clone 内部断言 → 可观察（deepcopy 句柄共享后备读同一文件） |
...
| test_generic_value_identity.py | 值身份/特化 + deep_clone 内部 | 契约 | ✅ 契约层保留（序列化往返）；deep_clone 内部断言 → tests/behavior/test_deep_clone_behavior.py（可观察） |
...
| test_storage_model_dispatch.py | 存储模型分发 | 契约 | ✅ 契约层保留（序列化分支）；deep_clone 内部断言（内存 list/磁盘协议/media 后备）→ 行为层可观察 + file_handle 可观察测试承接 |
