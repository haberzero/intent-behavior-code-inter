...
> **状态标记**：**F1（PT-DEBT-12）✅ / B1（PT-DEBT-13）✅ / F2（PT-DEBT-14）✅ / F3（PT-DEBT-14）✅ /
> M4（PT-DEBT-15）✅ 均已完成（2026-08-08）**；**M3（prompt 单源）已确认收敛**；
> **M1（.call 双写收敛）/ M2（驱动去重）已完成（2026-08-09，独立分支 exp/async-m1m2，
> 全量 2137 passed / 1 skipped）**。**全部 F1-F3/B1/M1-M4 收尾——统一执行模型闭环完成**。
> **最后更新**：2026-08-09
...
> **2026-08-09 已全部根治**：F1/B1/F2/F3/M4 落定后，本文件所述妥协点（M1 同步 `.call` 双写、M2 独立驱动）
> 亦已收敛（见 §二 落地状态）——**统一执行模型闭环完成**。本文件为临时任务文档，收尾后删除。
...
> **落地状态（2026-08-09）**：**1（F1）✅ / 2（B1）✅ / 3（F2）✅ / 4（F3）✅ / 6（M4）✅ 已完成；
> M3 已确认收敛**。**5（M1）/ 7（M2）已完成（独立分支 exp/async-m1m2，全量 2137 passed / 1 skipped）**——
> M2 线程体 `_drive_generator` 复用 `_drive_loop_gen` + `TaskScheduler`（单一权威驱动）；
> M1 四个可调用对象 `.call()` 变薄宿主包装委托 CPS 生成器 + `_drive_generator`（消绑定双写）。
> **全部收尾，统一执行模型闭环完成**。
