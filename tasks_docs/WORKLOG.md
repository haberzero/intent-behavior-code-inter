...
## 阶段 D6：Python 参考内核退役范围界定（2026-09-11，定论）

**"参考内核退役" = 参考角色退役，非 Python 运行时退役**（实证路由：数据面源 +
6 模块宿主调用 → Rust；LLM/宿主面源 → Python——"双内核协议"单源归属纪律）。

- **已退役（本阶段完成）**：① 差分数据面执行对拍（D2-⑤——语料契约权威化）
  ② Rust 侧 RUN_* 直写（D3——全语义码承载）③ 语义转录文化（D1 角移除 +
  D2 桥接——Rust = 语义权威，Python 语义实现不再为参考）。
- **保留（宿主执行层——职责重定位）**：① Python VM（LLM/宿主面执行——
  async/thread/ihost——"宿主面保留"）② Python 前端（编译器——共享服务，
  Rust 消费 artifact）③ 前端层差分（lexer/parser/deserializer/UID——D4 规范
  稳定实证后退役）。
- **待退役（D4 前置）**：前端层差分 + Rust 前端权威化（⑦ 终点最终断言）。
