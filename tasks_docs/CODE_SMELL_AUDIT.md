...
> **状态**：周期事实回顾完成（2026-08-09 全量 A/B/C/D 数据行核验定案：A2-A15/B7/C2-C13/D1/D2；A7/A15 失效确认，多数为设计内协议/分层/可选字段/容错解析，C6 已解决）。无遗留待核验项。独立分支（不与主线混置）
...
| # | 位置 | 特征 | 状态 |
|---|---|---|---|
| D1 | 15 处 `getattr(x, ..., None) or []` 模式 | 防御性兜底（`core/` 内） | **已核验：设计内可选字段处理（2026-08-09）**——全部 15 处均为 dataclass 可选字段（node.body/orelse、spec.param_types/param_descriptors、member 字段）在节点/spec 类型间的合法缺失态处理，非恒真恒假守卫、非防御兜底 |
