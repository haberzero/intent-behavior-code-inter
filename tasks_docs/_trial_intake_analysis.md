...
>（P0 三线：诊断面打包 → PT-FEAT-16 四批 → N2 已验证知识注册表实施[K1-K9 已确认]；P1×2 / P2×4 / 挂起×1）
...
- **已验证知识注册表**（试用方黑话原称“结晶”，用户 2026-09-07 裁定废除此名；第二轮新增 P0 机制，N2）：验证过的答案存起来，下次不再问模型
  （"用得越久越确定"）。
...
| **N2 已验证知识注册表**（试用方建议 API `ai.crystallize/lookup/correct/crystal_log` 为审查输入，黑话名已废除；引擎内路由：registry 命中→跳过 LLM；铁律：原始 LLM 输出不可直接登记，必须过确定性谓词；append-only 事件流） | 试用方 e25/e31 机制实证收益路径（fast path + 模式匹配 few-shot）；存储面与 IBCI 平铺池+UID 侧表同构 | **做（实施就绪）**（语言自动机使命核心机制——D 纸带知识层，"用得越久越确定"的机器承载）：设计文档 `_knowledge_registry_design.md` 已产出 + **K1-K9 已按推荐确认定案**（2026-09-07：类型 `knowledge` / 方法面 store 族 / 一等内置值类型[公理层] / 诊断域 `KNW_`；不以 ai 为载体、`@~...~` 保持纯 LLM 语义）→ 批次 ①-④ |
...
2. **已验证知识注册表（N2，试用方黑话原称"结晶"，已按用户 2026-09-07 裁定废除该名）**——"用得越久越确定"的机器承载：验证过的答案（LLM 提案 +
...
2. **N2 已验证知识注册表设计**：设计文档已产出（`tasks_docs/_knowledge_registry_design.md`，
    **2026-09-07 用户裁定重新定位**：不以 `ai` 模块为载体、行为描述语句保持纯 LLM 语义、
    ...
    ai 的 4 个 API；方法面（store/get/amend/history/keys/len，命名已定案）；验证门机器
    ...
    **K1-K9 已按推荐全量确认（2026-09-07 用户）**：类型 = `knowledge`、方法面 =
    store/get/amend/history/keys/len、形态 = 一等内置值类型（公理层）、诊断域 = `KNW_`
    → **实施就绪**（批次 ①-④）。
...
10. **N3 measure_freq（logprob 通道）**：measurement 类能力的机器承载；
    ...
    待 embedding/知识注册表面落地后重估（铁律三类操作中唯一尚无机器承载的一类）。
