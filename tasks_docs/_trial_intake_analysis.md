...
>（P0 三线：诊断面打包 → PT-FEAT-16 四批 → N2 结晶注册表设计；P1×2 / P2×4 / 挂起×1）
...
- **结晶**（第二轮新增 P0 机制，N2）：验证过的答案存起来，下次不再问模型
  （"用得越久越确定"）。
...
2. **结晶注册表（N2）设计**：设计阶段文档（`tasks_docs/_crystallize_design.md`）——
   ...
   （结晶库是否进 snapshot？——倾向：注册表 = 引擎级状态，随 save_state 持久）。
...
10. **N3 measure_freq（logprob 通道）**：measurement 类能力的机器承载；
    ...
    待 embedding/结晶面落地后重估（铁律三类操作中唯一尚无机器承载的一类）。
