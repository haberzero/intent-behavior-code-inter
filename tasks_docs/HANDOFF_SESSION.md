...
> `tasks_docs/HANDOFF.md` + **`tasks_docs/_trial_edge_catalog.md`（恶意试用起点边界清单）**。
...
> **起点清单**：`tasks_docs/_trial_edge_catalog.md`（接手者基于阶段 A/B 深度代码工作整理的
> 33 项已知+推测边界，覆盖意图/LLM 可调用类/协议族/覆层/宿主绑定/动态宿主/类型系统/控制流/
> 文档一致性）。**该清单是起点不是穷尽**——恶意试用必须以它为底，自行扩展边界，穷尽清单
> 未列出的场景（§6.4 各方向 + 自行挑刺）。
> 具体方向（在既有批判试用精神上扩展）：
