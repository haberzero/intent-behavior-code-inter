...
> 更早期的详细日志见 `tasks_docs/HISTORY_LOG.md`。
> 设计与实现细节见对应正式文档：`docs/architecture/03_type_system.md`、`docs/architecture/04_vm_interpreter.md`、`docs/architecture/05_vm_specification.md`、`docs/architecture/01_principles.md`。
> 当前最紧要项见 `tasks_docs/NEXT_STEPS.md`；阻塞项见 `tasks_docs/PENDING_TASKS.md`。
>
> **最后更新**：2026-07-21（修复 `ibci_sdk/check.py` `_load_module` 的 `FileFinder` 缓存陈旧 bug；下一项：media Phase 4，见 `NEXT_STEPS.md` GATED）

---

## 2026-07-21：修复 ibci_sdk/check.py FileFinder 缓存陈旧 bug

> 分支：`unsafe-vibe-dev`；测试基线：**1176 passed, 7 skipped**（连续 3 次全量跑 0 failure）。

### 根因
`ibci_sdk/check.py` 的 `_load_module` 在加载 `__init__.py` 时调用 `importlib.import_module(pkg_name)`，Python 的 `FileFinder` 会缓存 `parent_dir` 的目录列表到 `sys.path_importer_cache`，仅在目录 mtime 变化时刷新。`tests/sdk/test_check_plugin.py` 用 `tempfile.TemporaryDirectory()` 在同一个系统 temp 目录下快速创建/删除子目录，Windows 的目录 mtime 分辨率约 1-2 秒，短时间内的增删不触发缓存失效，导致 `importlib.import_module` 找不到新创建的临时包，报 `Failed to import __init__.py`。

### 修复
`ibci_sdk/check.py` `_load_module`：在 `importlib.import_module()` 调用前添加 `importlib.invalidate_caches()`，强制刷新 `sys.path_importer_cache`。

### 验证
- 复现脚本：修复前 5/10 失败，修复后 0/20 失败。
- `tests/sdk/test_check_plugin.py` 连续 5 次跑 0 failure。
- 全量 `python -m pytest tests/` 连续 3 次跑 0 failure，`1176 passed, 7 skipped`。
...
### E. 文档更新
- `docs/architecture/03_type_system.md` 更新 `TypeDef` 字段描述与 `SpecFactory` API。
- `docs/architecture/02_metadata_ast.md` §2.4 已登记三枚举（G1 WIP 中已落）。
- `tasks_docs/NEXT_STEPS.md` 移除 G1.5 条目，路径整合收尾（PT-ARCH-21-FU）提升为当前最紧要。
...
- 新增 `tasks_docs/decisions/ADR-017-path-module-layering.md`（Accepted，最高优先级）。
- 更新 `tasks_docs/decisions/README.md`（+ADR-017）。
- 更新 `tasks_docs/NEXT_STEPS.md`（PT-ARCH-19 提为最高优先级 P0；P0-1 标注遗留违规；P0-2 后置）。
- 更新 `tasks_docs/PENDING_TASKS.md`（§九 新增 PT-ARCH-19 完整计划；PT-ARCH-11 标注修正）。
...
- 新增 `tasks_docs/decisions/ADR-016-variable-storage-model.md`（Accepted，上层治理）：确立 `storage_model` 类型级属性（memory-backed / disk-backed）；磁盘型变量基于路径系统建模；协议驱动分发（零 `if/else`）；`__prompt__` 族保持为内存型协议；磁盘型获得平行协议族；制动 `MediaStorage` 整体淘汰；废弃"Inter 层"命名。
- 修订 `tasks_docs/decisions/ADR-013-unified-response-parsing.md`：去除标志位 `if/else` 内部分支，改协议驱动分发；supersede ADR-009 的核心结论（单入口/单策略/无 execute_* 分叉）仍成立。
- 重写 `tasks_docs/decisions/ADR-014-media-storage-handle-backing.md`：砍除 `MemoryBacking`；存储模型升为类型级（ADR-016）；制动 `MediaStorage` 整体淘汰；潜伏 bug 处置并入 PT-ARCH-18。
- 更新 `tasks_docs/decisions/README.md`：新增 ADR-016；标注 ADR-013/014 已修订。
- 重写 `tasks_docs/NEXT_STEPS.md`：新 P0 序列＝**P0-1 路径统一**（ADR-015）→ **P0-2 存储模型架构**（ADR-016，PT-ARCH-17）→ **P0-3 media 重建**（ADR-014，PT-ARCH-18，潜伏 bug 随之统一修复）；工作模式定论追加第 4 条（禁止过程式硬编码分发）与第 5 条（潜伏 bug 不允许过渡修复）。
- 更新 `tasks_docs/PENDING_TASKS.md`：PT-ARCH-12/13/16 不再独立先行，折叠入 PT-ARCH-17/18；新增 PT-ARCH-17（存储模型基础设施）+ PT-ARCH-18（media 重建）；PT-ARCH-14/15/PT-DOC-12 重标为 P0-1 子项。
...
- 新增 `tasks_docs/decisions/ADR-013-unified-response-parsing.md`（Accepted；supersede ADR-009）。
- 新增 `tasks_docs/decisions/ADR-014-media-storage-handle-backing.md`（Accepted；阻塞于 ADR-015）。
- 新增 `tasks_docs/decisions/ADR-015-path-system-unification-as-prerequisite.md`（Accepted；media 强制前置 gate）。
- 更新 `tasks_docs/decisions/ADR-009-call-llm-raw.md`（Status → Superseded by ADR-013）。
- 更新 `tasks_docs/decisions/README.md` 索引（+ Superseded 段、+ Phase 4 Media Foundation Decisions 段）。
- 重写 `tasks_docs/NEXT_STEPS.md`（新 P0 阶段序列：P0-1 清债 / P0-2 路径统一 / P0-3 统一解析；media 改为 gated；置入"⛔ 工作模式定论"）。
- 更新 `tasks_docs/PENDING_TASKS.md`（新增 §九 PT-ARCH-11~16 + PT-DOC-12 前置技术债）。
...
- `tasks_docs/decisions/` 目录 + ADR-007~012（6 份决策记录）
...
详见 `tasks_docs/HISTORY_LOG.md`。主要里程碑包括：
...
- 类型系统正式设计：`docs/architecture/03_type_system.md`
- VM 与解释器正式设计：`docs/architecture/04_vm_interpreter.md`
- VM 公理化规范：`docs/architecture/05_vm_specification.md`
- 实现细节备份：`docs/architecture/01_principles.md`
- 意图系统：`docs/subsystems/01_intent_system.md`
- 架构原则：`docs/ARCHITECTURE.md`
- 当前已知限制：`docs/KNOWN_LIMITS.md`
- 历史详细日志：`tasks_docs/HISTORY_LOG.md`
