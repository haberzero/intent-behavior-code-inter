...
> **状态**：规划阶段（Phase 0）。**独立任务，较低优先级**（当前无主线；下一任务为 PENDING 反射排查，见 `NEXT_STEPS.md`）。
> **性质**：正式独立任务文档（非临时）。4 份调研原始报告见 `TEST_REFACTOR_REPORTS.md`。
> **决策上下文**：缺陷复查已全部完成；本任务独立推进，不与当前主线混置。
