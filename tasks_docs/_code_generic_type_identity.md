...
### 2.6 已确认边界（独立复核核实，✅ 2026-08-13 已全部收敛）

> **状态更新**：本清单全部边界已由 `_code_generic_value_convergence.md`（独立分支
> exp/generic-value-convergence 4 commits）收敛——统一"类型上下文 → 字面量"传递
> 机制覆盖全部容器字面量产生路径。**剩余已知边界**（独立窗口）：
> ① `-> auto` 返回/生成器 yield 泛型实参推断（auto 语义固有局限）；②
> `-> generator[T]` 显式标注二次包裹（预存缺陷 c8b89564）；③ `*expr` 展开实参
> 静态不可绑定（根本限制）。

1. **函数返回字面量**：`func f() -> list[int]: return [1,2]` → `type(f())` = `list[int]`（✅ 已收敛）。
2. **函数调用实参字面量**：`f([1,2])`（形参 `list[int]`）→ `type(items)` = `list[int]`（✅ 已收敛）。
3. **下标/属性赋值字面量**：`m[0] = [9]`（`m: list[list[int]]`）→ `type(m[0])` = `list[int]`（✅ 已收敛）。
4. **嵌套容器内层元素**：`list[list[int]] n=[[1],[2]]` → 内层 `n[0]` = `list[int]`（✅ 已收敛，递归 helper）。
5. **容器切片**：`li[0:2]` = `list[int]`（✅ 已收敛，__getitem__ slice 用 self.ib_class）。
6. **Optional 值身份**：`Optional[int]` 值 = `Optional[int]`（✅ 已收敛，_wrap_optional 特化类）。
7. **跨引擎反序列化**：目标引擎 spec_reg 无该特化时经序列化 type_pool 重建（✅ 已收敛，include_static=True 契约）。
