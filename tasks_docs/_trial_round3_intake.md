...
7. **R3-⑦ 诊断消息批** ✅ **已完成（2026-09-08）**：D-6 顶层缩进 PAR 诊断附定向
   ...
   零回归。
8. **R3-⑧ D-7 裸声明语义** ✅ **已完成（2026-09-08，裁定 (c) 编译期检查）**：
   语义面实证——语句域裸声明 `int x` 无合法运行期语义（声明语句执行即抛
   RUN_TYPE_MISMATCH；"从不读"同样失败 = 死语法）；对照合法形态：类字段
   裸声明 = 构造器必填参数 / for 循环变量 / 函数形参。裁定 (c) 依据：
   fail-first（无合法运行期语义的语法不应到达运行期）+ 一致性（类字段已
   编译期门禁）+ (c) 包含 (b)（编译期精确定位声明语句）+ (a) 零值缺省 =
   魔法默认否决。实施：新码 SEM_DECLARATION_WITHOUT_INITIALIZER（纯增面 +
   catalog + 15_diagnostics + parity 门）+ visit_IbAssign 前置检查
...
### 原队列顺延（保留）✅ **Phase E 终态裁定（2026-09-08）**

- 原批 2 剩余：**A2 泛型约束 / A5 解构 / B2 惰性结构 = 挂起 → VISION-4 整合推进**
  （类型层语义项，与 VISION-4 类型理论加固同域；实施待 VISION-4 设计落地后推进，
  不半接通）
- 原批 3：**A6 Enum 增强 = 挂起 → VISION-4**（ADT 同域）/ **B5 并发成熟化 = 挂起**
  （一等原语已成熟[14_concurrency + howto + KNOWN_LIMITS 边界完整]，专项需单独立项；
  流式观测边界记 KNOWN_LIMITS 待评估）/ **C6 流式编排 = 完成**（多模型组合编排
  模式 howto：强弱分工/跨模型校验/扇出聚合，orchestrate_llm_calls.md 新增节 +
  关键语义澄清[behavior 型容器元素/retry 保留词]，三模式示例实测验证）/
  **C7 性能内省 = 完成**（R3-⑤ 消重）/ **D2 CLI 导出 = 完成**（check --format json
  结构化诊断导出，判别 4 项）/ **D3 配套诊断码 = 挂起**（随新能力配套，非独立批）
