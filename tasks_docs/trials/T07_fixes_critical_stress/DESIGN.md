...
| 约束 | 内容 |
|------|------|
| 死循环保护 | 每例经 harness 硬超时 SIGKILL 进程组 + --max-inst 双层兜底；无超时不运行（--timeout 必填） |
...
python tasks_docs/trials/T07_fixes_critical_stress/rerun_old_suites.py --timeout 60 --parallel 4
```
