...
> **队列性质**：完成"trial 试用者需求 + 已知近期主线"的大任务。本表是**基线队列**；
> **执行 agent 拥有相当程度的自由任务规划权**（用户 2026-09-07 指示）：可基于实际
> 执行、实时代码分析与开发进度状态，自主微调进度顺序、优先级与批次划分（调整理由
> 记入工作日志），但不得把队列外的新大任务拉入主线（新发现按 INDEX/WORKLOG 登记
> 后归入相应批次）。
> **恒高优先原则**（用户 2026-09-07 指示）：**来自真实试用者（ibci-trial 反馈 /
> 早期试用者质询）的需求始终是优先处理的高优先级选项**——自由微调的行使不得系统性
> 地把试用者真实需求排到自研/卫生项之后；冲突时试用者需求优先。
> **每项出口标准（统一）**：设计阶段文档（如适用）→ 实现 → 全量 pytest 零回归 →
> 落账（INDEX/WORKLOG/本文件）→ 该项转"完成"。同一时刻只主推一个 P0 项。

### 5.1 P0 主线（三条线；基线顺序 P0-1→P0-2→P0-3，可按自由任务规划权微调；每线可含多个批次）
...
**第一个动作（默认，可按自由任务规划权基于实时分析调整）**：写 `tasks_docs/_diagnostic_design.md`（P0-1 设计文档）
...
（WORKLOG/INDEX/handoff）闭环。每轮自主评估现状与实时代码/开发进度状态、自主决定推进哪一项，并拥有**相当程度的
自由任务规划权**（§五 为基线队列非固定脚本；可微调顺序/优先级/批次划分，调整理由
记入工作日志；**恒高优先：来自真实试用者的需求始终优先处理**）。队列有活持续推进
——不设固定停止线；仅当 §五 6 的收敛判据成立（全部项处于完成/挂起/裁定不做终态）
