...
| **🟢 下一阶段主要目标（2026-08-12 用户提出，设计起点已落档 `TRIAL_SYSTEM_DESIGN.md`）** | **试用体系规范化** | 工程/验证 | **试用机制体系化 + 日志体系化 + 历史记录整理定型**。现状：4 套试用地基（`_LLM_TRIAL_20260812`/`_LLM_TRIAL_ENUM_IMPORT_20260812`/`_GENERICS_TRIAL_20260812`/`_GENERICS_TRIAL_FIX_20260812`）harness 与 register.jsonl 已统一（md5 一致、15 字段），但命名/分类体系（LLM 7 类 vs 泛型 4 类）/REGISTER 格式/缺陷编号（`KERNEL-ISSUE-001` vs `-G1`）碎片化，harness 物理 4 份复制非单一权威源。**规范化**：① 单一 harness（`trials/_toolkit/` 引用非复制）+ CLASSIFICATION.md 统一分类（PASS/GUARD/KERNEL_ISSUE/BOUNDARY/DOC_ISSUE/LLM_BEHAVIOR/LIMIT/HARNESS）+ 级别 P0-P3；② 统一目录骨架（DESIGN_TEMPLATE/REGISTER_TEMPLATE）+ 缺陷编号 `KERNEL-ISSUE-<域>-<n>`；③ 历史 4 套 git mv 迁移 + REGISTER 重排 + 编号映射表 + `trials/INDEX.md`。**Phase 1 机制规范化 → Phase 2 历史定型 → Phase 3 收尾**（全量 pytest 零回归，仅文档/脚本不涉内核）。**与 G3/BOUNDARY-G2 修复（`_HANDOFF_GENERICS_FIX.md`）错峰**。设计 `TRIAL_SYSTEM_DESIGN.md` |
...
| **🟡 待修复（2026-08-12 修复回归试用发现，`_GENERICS_TRIAL_FIX_20260812/REGISTER.md`）** | **KERNEL-ISSUE-G3 继承特化 + 父类字段值丢失** | 架构健康性 | **现象**：`class Linked[T](Node[T])` → `Linked[int](5).get()` 返回 None（静默值丢失，非崩溃）。**已用 git worktree 在修复前（35bb2de）复现**——PT-FEAT-3 既有缺陷，非本次修复引入。**根因方向**：继承特化类（`Linked[int]`）的字段绑定/hydration——父类 Node 字段在特化继承链上未正确初始化（`Linked[int]` 特化类 default_fields 缺父类字段或 `instantiate` 沿链收集缺失）。影响：**继承特化的字段值全部丢失（严重，静默）**。**P1** |
| **🟡 待修复（2026-08-12 修复回归试用发现，`_GENERICS_TRIAL_FIX_20260812/REGISTER.md`）** | **BOUNDARY-G2 自引用链 while 遍历运行时类型退化** | 架构健康性 | **现象**：`Node[int] cur = nodes[0]; while cur is not None: total += cur.get(); cur = cur.next` 运行时 `cur` 类型退化 any → `AttributeError: any has no attribute 'get'`（R5-04）。运行时类型跟踪问题：自引用字段 `cur.next` 回写变量后类型信息丢失。**P2** |
