...
| **🟡 待核实修复（2026-08-13 探索发现，交接文档 `HANDOFF_GENERIC_ASSIGNABILITY.md`）** | **内置泛型赋值类型检查缺失泛型实参校验** | 架构健康性 | **现象**：`thread[int]` 可赋给 `thread[str]`、`list[int]`→`list[str]`、`dict[str,int]`→`dict[str,str]` 编译期全部通过（应报 SEM_TYPE_MISMATCH）。**实测 10/11 类内置泛型受影响**（thread/thread_result/chan/slot/generator/fn_callable/behavior/list/dict/tuple）；Optional 与用户类泛型（Box[int]）正确拦截。**根因**：`is_assignable`（`_assignability.py:72`）的 axiom 兼容分支 `src_axiom.is_compatible(target.name)` 先于 :62 name 比较命中；所有内置泛型 axiom `is_compatible` 用 `startswith("类型[")` 前缀匹配（12 处），忽略泛型实参。**P1**（编译期类型安全漏洞，错误类型静默流入）。修复方向见交接文档 §1.4（方案 A：is_assignable 先做泛型实参比较；方案 B：12 处 axiom 改结构化比较），按 Phase D 收敛义务双交付 |
| **🟡 待设计修复（2026-08-13 重新分析升格，交接文档 `HANDOFF_GENERIC_ASSIGNABILITY.md` §二）** | **内建泛型运行时值层类型擦除（初期设计缺陷/架构妥协）** | 架构健康性 | **现象**：`type(Box[int]值)`→`Box[int]`（用户类泛型带实参），`type(list[int]值)`→`list`（内建泛型无实参）。**根因**：编译 `list[int] li` 后 spec_reg 有 `list[int]` 特化 spec 但 `get_class("list[int]")` 为 None——**特化 spec 从未水化为运行时特化类**；`factory.create_list`（:50）硬编码 `get_class("list")` 基类，`IbValue.type_ref` 从基类 spec 构造 → 擦除。**定性**：初期设计妥协（内建容器早期用单类+擦除最快实现），用户类泛型（2026-08-12）引入 create_subclass 特化类后**未回填内建泛型**——双轨不对称（design-philosophy §二/§四违反）。**实际后果**：① `type()` 内省分裂；② 值层无法区分 `list[int]`/`list[str]` → 运行时类型安全缺失（与缺陷一联动：`list[str]` 传 `list[int]` 参数编译期放行、运行时 `RUN_TYPE_MISMATCH`）；③ 运行时内省能力缺失。**处置**：根治=内建泛型特化 spec 水化为特化类（factory/`_specialize`/序列化值层带实参，独立设计窗口）；近期=缺陷一独立修复阻断错误延迟 + 文档现状说明。**P1-P2** |
