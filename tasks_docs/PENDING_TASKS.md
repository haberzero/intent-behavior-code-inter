...
| **🟡 已登记待修（2026-08-14，T07 批判性试用新暴露，只登记不修复）** | **KERNEL_ISSUE-OPTIONAL-SCOPE-1：函数作用域内 Optional 先 None 后赋值，unwrap()/is_some() 报 `Object of type None`** | 语义正确性 | 触发 `trials/T07_fixes_critical_stress/cases/D2-09.ibci`。`func work() -> int: Optional[int] tag = None; tag = 405; return tag.unwrap()` → `[ERROR][RUN_ATTRIBUTE_ERROR]: Object of type None has no method __call__`；**顶层同代码成功（405）、lambda 内 is_some() 成功、`return tag` 成功**——仅普通函数作用域“先 None 后赋值”路径失败（线程 worker 内同失败，ThreadFailed 包装）。文档 `arch/03_type_system.md` §8 “is_none()/is_some()/unwrap()/or_else() 在任何路径可用”不成立。**P1**，根因分析留待后续任务（本任务不查内核不修）。见 `trials/T07_fixes_critical_stress/REGISTER.md` §二。 |
| **🟡 已登记待修（2026-08-14，T07 批判性试用新暴露，只登记不修复）** | **KERNEL_ISSUE-OPTIONAL-CONTAINER-1：Optional[list[int]] 有值包装后 len()/下标不可用** | 语义正确性 | 触发 `trials/T07_fixes_critical_stress/cases/D2-10.ibci`。`Optional[list[int]] b = [1,2,3]; print(len(b))` → `RUN_ATTRIBUTE_ERROR: Object of type Optional[list[int]] has no method len`；`b[1]` → 无 `__getitem__`；`b is None` → False（有值包装正常）。文档 §8 “Optional[T] 接受 T”语义下容器操作断裂。**P1**，根因分析留待后续任务。见 REGISTER §二。 |
| **🟡 已登记待修（2026-08-14，T07 批判性试用新暴露，只登记不修复）** | **KERNEL_ISSUE-ATTR-READ-1：未声明属性读取静默返回 None（仅调用路径报 RUN_ATTRIBUTE_ERROR）** | 语义正确性 | 触发 `trials/T07_fixes_critical_stress/cases/D1-13.ibci`。`Point p = Point(1); print(p.y)`（y 未声明）→ 输出 None（exit 0），不触发 RUN_ATTRIBUTE_ERROR；`p.missing_method()` → RUN_ATTRIBUTE_ERROR。文档 `15_diagnostics.md` RUN_ATTRIBUTE_ERROR “触发条件：访问对象不存在的属性/方法”与实现不符（读取路径未覆盖）。**P1（静默错误值）**，根因分析留待后续任务。见 REGISTER §二。 |
| **🟡 已登记待办（2026-08-14，T07 D5 文档核验，正文修改待用户确认）** | **DOC-24~28 文档同步批次（与 3 项 KI 同源 + KNOWN_LIMITS §10.2 + 14_concurrency）** | 文档健康 | T07 文档核验产出：DOC-24（arch/03_type_system §8 `unwrap/is_some/is_none 在任何路径可用`不成立，同 OPTIONAL-SCOPE-1）、DOC-25（§8 Optional 容器方法不可用未说明，同 OPTIONAL-CONTAINER-1）、DOC-26（15_diagnostics RUN_ATTRIBUTE_ERROR `访问对象不存在的属性/方法`触发条件与实现不符——读取路径静默 None，同 ATTR-READ-1）、DOC-27（KNOWN_LIMITS §10.2 建议补 qualified 路径实证——裸名退化表述仍成立，T07 D3 实证 qualified 全生效）、DOC-28（14_concurrency `chan(T, name, ...)` 宜补充说明：特化类对象实参降级为裸类，普适写法 `chan[E] ch = chan()`，见 BOUNDARY-CHAN-ARGS-1）。**处理纪律**：doc-governance Phase 0-8，修文档保持实现（先例：DOC-1/2 mock 值语义决断）；其中 DOC-24/25/26 须与 3 项 KI 修复联动（修复后同步措辞）。 |
