...
| 优先级 | 任务 | 维度 | 说明 |
|--------|------|------|------|
| **🔴 P0（下一 session 主线，用户 2026-08-14 提升优先级）** | **函数/可调用类型身份架构断层根治（三个深层次缺陷同源）** | 架构健康性 | 用户猜想"fn 设计早于泛型体系存在历史包袱"**已证实**。三个问题经代码级实证指向同一架构断层：**编译期/运行期"类型身份"在函数与可调用类型关键路径上被扁平化/丢失**——`fn`/`callable` 函数签名建模基于早期"字符串级 TypeRef"设计，泛型地基（TypeRef 结构化 + CALLABLE_SIG + CALLABLE_INSTANCE）后 `create_func`/`resolve_member` 未同步升级。**三个问题**：① **绑定方法建模缺陷**——`resolve_member`（`_members.py:43-50`）对方法成员无条件建模 FUNCTION kind，`BoundMethodAxiom`（bound_method IS-A callable，`callable.py:32`）编译期从未接线 → `a.calc` 赋 callable 被拒、编译/运行期身份不对称；② **函数符号 spec 回填签名丢失**——`_declaration_visitors.py:123-131` 用 `create_func` 重建 spec 只取 `p.name`/`ret_type.name` 字符串 → `-> fn[(int,str)->int]` 退化为裸 fn、泛型/Optional 返回签名丢失（create_func 8 调用点）；③ **函数返回值 Optional 包装缺失**——`vm_handle_IbReturn`（`control_flow.py:118-122`）`yield value_uid` 原样返回不做 wrap → 链式 `maybe(5).unwrap()` 报 int 无 unwrap。**与既有地基分析同源**：PENDING_TASKS 第 74 行 `_DEEP_ANALYSIS_TYPE_SYSTEM_FOUNDATION.md` 已指出"类型身份自始是扁平字符串名单一权威 + TypeRef 平行双轨"——本三项是该地基缺陷的**函数/可调用类型侧面**（此前深挖聚焦泛型容器）。**交接文档**：`_HANDOFF_TYPE_IDENTITY_FAULT_LINE.md`（含逐环实证根因 + 修复方向 + 判别性回归建议）。**修复性质**：破坏性重构（create_func API 升级影响 8 调用点）→ 独立分支实验，确认零风险后更新 unsafe-vibe-dev。 |
