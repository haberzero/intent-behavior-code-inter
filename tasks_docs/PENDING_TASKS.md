...
| 域 | 活跃 | 搁置 | 封存 | 说明 |
|----|------|------|------|------|
| FEAT（功能） | 4 | 0 | 0 | 语言/工具链功能愿景（含 PT-FEAT-15 原生宿主绑定主干） |
...
### PT-FEAT-15 IBCI 原生宿主绑定重构（路线 X，当前主干）

- **状态**：active（规划交接阶段）｜**域**：FEAT｜**优先级**：P0
- **动机**：抛弃"用户在 Python 侧手写 `_spec.py` 暴露库给 IBCI"的插件思路，改为 IBCI
  用户代码层原生包装 Python 内容（宿主导入一等语法 + 用户用 IBCI 类型/协议/impl 绑定
  Python 成员 + `impl`/协议扩展到宿主类型 + 插件体系按新思路重构）。
- **成因**：用户裁定（2026-08-17）；深度调研确认 `box()` 已能包装任意 Python 对象/可调用，
  但 `import X` 与 `impl` 目前受 `_spec.py` / 本模块用户类限制。
- **当前理解**：总路线图见 `tasks_docs/ROADMAP_NATIVE_BINDING.md`（现状实证 / 阶段 P0-P5 /
  关键裁决点 §四）。P0 基线固化后按 P1→P5 推进。
- **关联**：`tasks_docs/HANDOFF.md` §2；`tasks_docs/WORKLOG.md` §三（路线 X 裁定）。
