...
| **🟡 待修复（2026-08-12 泛型试用发现，`_GENERICS_TRIAL_20260812/REGISTER.md`）** | **KERNEL-ISSUE-G1/G2 泛型类自引用特化缺失** | 架构健康性 | **根因已定位（单根因）**：泛型类自引用（方法体内 `Box[T]` 构造/返回 / 自引用字段 `Node[T] next`）在 symbol_collection 阶段被**扁平化**为 `TypeRef('Node[T]')`（head 含方括号字符串、args 空），`TypeRef.substitute` 只认结构化 args 无法替换 → 特化 spec 中自引用类型仍为 `Node[T]`。**G1**：方法体 `Box[T]` 运行期 `Variable UID 'Box:T' is not defined` / 编译期 `Cannot assign 'Vec' to 'Vec[int]'`（D2-01，t1 探针）。**G2**：`Node[int].next` 字段 type_ref 仍 `Node[T]` → `n1.next = Node[int](2)` 报 `Cannot assign 'Node[int]' to 'Node[T]'`（D3-04）。**修复方向**：`_resolve_annotation`/`_annotation_to_typeref` 对 `Node[T]`（value=当前类/已注册泛型类）产出结构化 `TypeRef('Node', (TypeRef('T'),))`（内层 T 经 `_lookup_type_param` 解析）。影响：链表/树/工厂方法/迭代器等自引用泛型模式不可用。**P1** |
