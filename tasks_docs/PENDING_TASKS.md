...
| **当前主线（下一 session，2026-08-12 用户升主线）** | **用户类泛型参数（PT-FEAT-3 升主线）** | 易用性+架构 | **用户裁定（2026-08-12）：用户类泛型提升为主线任务**。`class Box[T]:` 语法/语义/序列化扩展。地基已备（`GenericTypeRegistry` 支持任意泛型声明，create/parse/serialize/restore 全链路）。范围：① 语法层 `type_params`（lexer/parser/AST `IbClassDef.type_params`）；② 语义层（类型参数符号 + 特化解析 `resolve_specialization` 对用户类）；③ 序列化（type_params 落 artifact + rehydrate）；④ e2e（`Box[int]`/`Box[str]` 特化实例化/成员方法/与内置泛型同构）。**设计冻结先行**（`tasks_docs/`），落地后按 docs 治理写技术手册。当前无阻塞 |
...
| **P2** | **F9 配置副作用与 import ai 耦合评估**（T4 审计发现） | 设计评估 | 真实配置 + 未装 openai 时 `import ai`/引擎启动即抛 RuntimeError（setup 自动加载 api_config.json → apply_config → set_config → _init_client）。**用户裁定方向（2026-08-12）：改为显式配置**——提供 `ai.autoset`（或等价命名）类方法，明确操作后果，用户在主函数/入口显式调用；引擎启动不再自动加载。**设计记录 `_code_ai_autoset.md` 已产出**（现状分析 + 显式方案 + 变更清单 + 安全契约归位 + 命名候选）。**待用户拍板：方法命名（`ai.autoset` / `ai.load_project_config` / `ai.configure`）**，确认后实施（全仓示例/测试/文档更新） |
