...
| **P1（异步统一完整）** | **异步统一遗留 A1-A6**（`_HEALTH_AUDIT_PLAN.md`） | 架构健康性 | 地基闭环后 6 处**次要路径**仍任务内同步重入/嵌套调度器：**A1（内联 `@~`，llm_behavior.py）✅、A3（`_SlotUpdateWaitable`，comm.py）✅、A4（LLM 函数同步阻塞，_llm_function.py，PT-FEAT-1 直接项）✅ 均已完成（2026-08-10，独立分支 exp/async-unify-a，全量 2138/1）**；**A2** 意图消解（intent.py:59，`vm.run` 重入，可独立窗口）；**A5** 类构造（ib_class.py:128/154）、**A6** 协议方法 `.call`（条件触发）涉及语义边界需独立窗口 |
...
| PT-DEBT-16 | 异步统一完整性遗留（A1-A6，`_HEALTH_AUDIT_PLAN.md`） | **地基闭环后 6 处次要路径仍任务内同步重入/嵌套调度器（2026-08-09 只读审计）→ A1/A3/A4 已完成（2026-08-10，独立分支 exp/async-unify-a，全量 2138/1）**：**A1** 内联 `@~` 表达式接 CPS（llm_behavior.py 切 `execute_behavior_expression_cps`，顺带补 `IbGenerator.generic_next` Waitable 契约缺口）；**A3** `_SlotUpdateWaitable` 增 `cps_drive` 帧内驱动（`vm_handle_IbCall` 识别并 `yield from`，消除嵌套 TaskScheduler）；**A4** LLM 函数 CPS-yield（`_prepare_llm_function_call_cps`+`_call_and_parse_llm_function` worker 化，PT-FEAT-1 直接项）。**剩余**：**A2** 意图消解 `resolve_content`/`get_resolved_prompt_intents` 内 `vm.run` 同步重入（intent.py:59，与 A1 同性质）；**A5** 类构造 `instantiate` 字段 `vm.run` / `init_method.call`（ib_class.py:128/154）；**A6** 协议方法 `.call` 条件触发嵌套（llm_parsing_strategy / llm_except_frame）。**处置**：A2 可独立窗口推进；A5/A6 涉及语义边界需独立窗口/用户裁决 |
