...
| **🟡 待修复（2026-08-13 试用体系规范化期发现，`tasks_docs/trials/T04_generics_fix_regression/`）** | **KERNEL_ISSUE-GEN-5 嵌套内置泛型实参的用户类特化运行时未注册** | 架构健康性 | **现象**：`Box[list[int]]`（内置泛型 list 作实参的用户类特化）在部分代码形态下运行时 `_specialize` 报 `Generic class 'Box' has no registered specialization for type 'list[int]'`（R5-04 组合用例 while 后的 Box 部分首次触发；`type(Box[list[int]])` 表达式形态独立触发）。**git stash 实证为预存缺陷**（与 2026-08-13 G3/Finding C 修复无关）。触发形态未全探明（while 循环存在 vs 无、表达式 vs 注解实例化存在差异）；疑似编译期 `Box[list[int]]` 特化 spec 注册条件性缺失。R5-04 用例已简化 Box 部分（`Box[int]`）规避。**P2**，独立窗口深挖 |
