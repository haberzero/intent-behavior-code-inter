...
| **🟡 待文档精确化（2026-08-13 探索发现，交接文档 `HANDOFF_GENERIC_ASSIGNABILITY.md` §二）** | **`type()` 泛型内省不对称** | 文档/内省 | **现象**：`type(Box[int]值)`→`Box[int]`（用户类泛型带实参），`type(list[int]值)`→`list`、`type(Optional[int]值)`→`Optional`（内建泛型无实参）。**根因**：`_type`（meta.py:17）返回 `obj.ib_class.name`；用户类泛型运行时是特化子类（name=Box[int]），内建泛型运行时是单一基类（factory.py:50 `get_class("list")`，值层类型擦除）。**判定**：非缺陷（内建容器值层单类设计合理，对齐 Python），但用户类 vs 内建内省形态分裂 + `12_builtins.md` 未注明差异。**处置**：① 文档精确化（低风险，补充"内建泛型返回基名、用户类泛型返回特化名"）；② 值层携带实参元数据（P2 远期，独立设计窗口）。**P3** |
