...
| **🟡 待修复（2026-08-12 泛型试用发现，深度核验后拆分，`_GENERICS_TRIAL_20260812/REGISTER.md`）** | **KERNEL-ISSUE-G2 泛型类自引用字段特化替换失效（编译期）** | 架构健康性 | **现象**：`class Node[T]: Node[T] next` 特化后 `n1.next = Node[int](2)` 报 `Cannot assign 'Node[int]' to 'Node[T]'`（SEM_TYPE_MISMATCH）。**根因（深度核验实证）**：① 字段注解 `Node[T]` 经 `_resolve_annotation` → `resolve_specialization(Node, [T占位])` 创建了 `Node[T]` 特化 spec（type_params=[]，污染注册表）；② 字段构造 `TypeRef.from_spec(Node[T] spec)` 对 CLASS kind 落**默认 fallback**（type_ref.py:234 `TypeRef(get_base_name())`）→ 扁平化为 `TypeRef('Node[T]')`（head 含方括号、args 空）；③ `TypeRef.substitute` 对扁平形态无法替换（head="Node[T]" 非 "T"、args 空 → 原样返回）。**定性**：`from_spec` 扁平化是**既有行为**（type_ref.py 本次零改动），用户类泛型首次触发暴露；非本次引入，但 PT-FEAT-3 设计决断 2 声明"嵌套与内置泛型同构"未覆盖自引用。**修复方向（已验证）**：字段注解 `Node[T]` 产出结构化 `TypeRef('Node', (TypeRef('T'),))`（`_annotation_to_typeref` 已是结构化，需让 `_resolve_annotation` 字段路径走结构化或修 `from_spec` CLASS 分支）。影响：链表/树/迭代器等自引用模式不可用。**P1** |
| **🟡 待修复（2026-08-12 泛型试用发现，深度核验后拆分，`_GENERICS_TRIAL_20260812/REGISTER.md`）** | **KERNEL-ISSUE-G1 泛型方法体内类型参数表达式运行时失效（运行期）** | 架构健康性 | **现象**：`func make(self, T v) -> Box[T]: return Box[T](v)` 运行期 `Variable UID 'Box:T' is not defined`。**根因（深度核验实证）**：编译期语义**正确**（方法返回类型 `-> Box[T]` 结构化 `TypeRef('Box',(T,))`；方法体 `Box[T](v)` 的 IbCall 类型绑定 `Box[T]` 正确；slice T 的 node_to_symbol 正确绑 TYPE_PARAM 符号，UID=`scope_.../Box:T`）。**运行期** `vm_handle_IbName`（leaf.py:75）按 UID 查 `runtime_context` 变量——类型参数 T 不产生运行时变量，UID 从未注册 → RUN_UNDEFINED_VARIABLE。**与 G2 是独立缺陷**（不同层：G2 编译期类型系统、G1 运行期符号解析）。**修复方向**：运行时对 TYPE_PARAM 符号 UID 的类型解析（`Box[T]` slice T 应解析为类型标识，对齐 `Box[int]` slice int 求值为 IbClass 的既有机制）。**P1** |
| **🟡 待修复（2026-08-12 泛型试用发现，`_GENERICS_TRIAL_20260812/REGISTER.md`）** | **BOUNDARY-G1 非法特化实参编译期未拦截** | 架构健康性 | `Box[42]`/`Box[None]` 编译通过，运行期 `_specialize` 抛裸 `AttributeError`（非诊断码）。**修复方向**：语义层 `_resolve_type` IbSubscript 分支对非类型 slice 报 SEM_GENERIC_TYPE_NEEDS_ARGS（或新码）。**P2** |
| **🟡 待修复（2026-08-12 深度核验发现，PT-FEAT-3 复核 H1 引入）** | **双通道设计缺陷：特化方法参数 descriptors 替换两套实现** | 架构健康性 | `_substitute_members`（_assignability.py，真实 mapping dict 驱动）与 `_sync_specialized_members`（_declaration_visitors.py，**字符串解析特化名反推 mapping**——`_type_param_mapping` 用 `_split_top_level_args`/`_parse_type_ref_name` 从 `"Box[list[int]]"` 反推）是同一语义的**两个实现**，违反工作模式定论第 3 条（不靠隐式字符串变换）+ design-philosophy §四（机制同构，同一语义两种实现）。后者是 H1 复核时补的症状层补丁，真正根因是 descriptors 回填（type_checking）晚于特化构造（symbol_collection）的时序。**修复方向**：消除 `_sync_specialized_members` 字符串反推，改为让 `_substitute_members` 在特化时用真实 mapping 处理 descriptors（需解决时序：基类 descriptors 回填后再触发特化类 descriptors 同步，或特化 spec 惰性解析）。**P2** |
