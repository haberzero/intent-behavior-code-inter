...
| **🟡 已登记待修（2026-08-14，T07 E 批判补充批次发现）** | **DOC-29 空 Optional 操作错误码与文档承诺不一致** | 诊断契约 | `Optional[T] e = None` 的 `for x in e` 迭代 / `next(e)` / `e.unwrap()` 抛 `RUN_GENERIC_ERROR`（iterable.py:39 / optional.py:90 抛 InterpreterError 未指定 error_code → 默认码）；而委托链空值路径（optional.py:174）与文档 arch/03 §8 承诺均报 `RUN_ATTRIBUTE_ERROR`——**同语义"空 Optional 操作不可用"两处码不一致 + 文档矛盾**（15_diagnostics RUN_GENERIC_ERROR 定义"未归类运行时错误" vs arch/03 §8 "空 Optional 操作 fail-fast 报 RUN_ATTRIBUTE_ERROR"）。base（30511d4f）同现空值 unwrap 亦 RUN_GENERIC_ERROR=pre-existing；修复②把 base 的裸 RuntimeError（迭代）改进为有码错误（进步）但码与文档承诺不符。**修复方向**：iterable.py/optional.py 空值错误补 `error_code="RUN_ATTRIBUTE_ERROR"`（或文档同步为 RUN_GENERIC_ERROR——需裁决）。触发用例 `trials/T07_fixes_critical_stress/cases/E1-empty-optional-code.ibci`/`E2-empty-optional-unwrap-code.ibci`。 |
| **🟡 已登记待办（2026-08-14，T07 E 批判补充批次发现，base 同现）** | **BOUNDARY-NESTED-FUNC-1 函数返回嵌套函数赋 fn_callable 类型 RUN_TYPE_MISMATCH** | 语义正确性 | `func outer() -> fn_callable[int]: func inner() -> int: ...; return inner` → 运行时 `RUN_TYPE_MISMATCH: Cannot assign 'callable' to 'fn_callable[int]'`（`_check_type` 把嵌套函数符号识别为裸 callable）；lambda 返回正常。**base（30511d4f）同现=pre-existing**，与 T07 修复①声称的 rehydrator kind 保真（FUNCTION/CALLABLE_SIG shell kind）无关——修复覆盖序列化水化路径，未覆盖运行时 `_check_type` 对嵌套函数值对象的判定。触发用例 `trials/T07_fixes_critical_stress/cases/E3-nested-func-return.ibci`。 |
