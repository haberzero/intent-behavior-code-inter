...
| **🟡 待设计（2026-08-13 用户提出，`LLM_SERVICE.md` 记录）** | **供应商感知的模型思考禁用机制** | 架构健康性 | **背景**：qwen3.6-35b-a3b 在 LM Studio 强制思考，`enable_thinking=false`/`thinking.enabled=false`/`chat_template_kwargs` 全参数实测无效（后端强制，API 无法关闭）。**现状**：IBCI 已发**思考禁用失败警告**（一次性去重，引导联系开发者/提交 issue 附供应商文档；不引导用户改配置绕开）。**待完善**：按供应商参数形态实现思考禁用/检测失败覆盖——LM Studio/llama.cpp（`chat_template_kwargs.enable_thinking`）、vLLM（`chat_template_kwargs`）、Ollama（模型模板）、OpenAI（`reasoning.effort`，Responses）、Anthropic（`thinking.budget_tokens`）、Gemini（`thinkingConfig`）等，逐供应商探测禁用有效性 + 失败警告。**P2**，独立设计窗口（配置 schema 扩展 + provider 分派） |
