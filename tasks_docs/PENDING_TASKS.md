...
>
> **最后更新**：2026-07-31（新增 PT-4.8 布尔上下文行为表达式定型 str 恒真陷阱——独立调研项，本轮主线完成后启动）
...
### PT-4.8　布尔上下文行为表达式定型为 `str` 的恒真陷阱 [DESIGN-DEBT] [待调研]

> 行为表达式嵌套在复合布尔表达式（如 `@~...~ and True`）中时被定型为 `str`，LLM 返回的
> `"0"` / `"false"` 按 Python 非空字符串真值语义判真，`while` 条件恒真可致死循环。
> 与 llmexcept 无关（不加 llmexcept 亦可复现）。
>
> **本轮次主要任务（llmexcept 机制统一）完毕后立即启动独立调研**：编译器类型推断
> 在布尔上下文中将行为表达式定型为 `bool`，评估破坏面（BoolOp/条件推断路径）后实现。
> 语言级限制见 `docs/KNOWN_LIMITS.md` §二十三。
