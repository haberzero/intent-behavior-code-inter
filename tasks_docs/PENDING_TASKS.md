...
| 优先级 | 任务 | 维度 | 说明 |
|--------|------|------|------|
| **当前阶段（重启，2026-08-12）** | **真实 LLM 全面试用 + 语法/语言功能评估重启**（`_REAL_LLM_E2E_RESTART.md`） | 验证/发布 | **重启** 2026-08-11 被中断的"真实 LLM e2e 全面试用"：原报告 `_REAL_LLM_E2E_REPORT.md` §四 9/12 类基于**含 U1-U7 不合格操作的旧代码**执行；意图机制结论已实证纠错（7339220）；修复后（U1-U7/意图纠错/T1-T5/PT-AUDIT-3/dispatch 观测 d6d28e1/run_batch 观测 df1a896）**未在真实 LLM 下重跑 12 类遍历**。重启 = §四 12 类遍历（重验 A1-A5：意图 @/@! 赋值路径 / 内建遮蔽+LLM 表达式 / generator.to_list / dispatch 赋值后 idbg 观测 / run_batch 观测）+ §五 批判检测（补 C1-C4：长提示 / 非确定性 / 超时断连 / 并发扩展）+ B1-B3 未测项补记录 + 刷新 `_REAL_LLM_E2E_REPORT.md` §四/§五/§七 |
| **当前阶段（已完成阶段 2 收尾；阶段 3 待重启通过 + 用户授权）** | **unsafe-vibe-dev 合并取代 main**（`_MAIN_MERGE_PLAN.md`） | 发布/稳定性 | 阶段 2 收尾准备完成（doc-health P1/P2 + pyproject 0.2.0 + examples 真实跑通 11 例 + `_MERGE_READY_REPORT.md`）。**前置**：真实 LLM 全面试用重启通过（`_REAL_LLM_E2E_RESTART.md`，使合并检测维度基于修复后代码确认）。**阶段 3（`git merge unsafe-vibe-dev → main`）待用户显式授权**（push/合并不在自主范围，禁 push 硬原则） |
...
| **P2** | **PT-DEBT-24 call_intent 预留机制核查**（T2 审计发现） | 架构健康性 | `IbBehaviorExpr` AST 无 intent 字段，`_prepare_behavior_call`/`invoke_llm_function_cps` 的 `call_intent` 短路路径（auto_intent=False 时）正常代码不可达——预留/遗留机制。低优先清理或删除待独立窗口（**PT-AUDIT-3 独立审计复核确认**：dispatch/run_batch 均不传 call_intent，`_behavior.py:150-162/238-252` 分支实际死代码） |
...
| **P2** | **PT-AUDIT-3 双路径分裂专项审计**（T1/T4 教训驱动，新增） | 架构健康性 | 意图缺陷（PT-DEBT-22）揭示"双路径语义分裂 + 快照半消费 + 自审计不可全信"三类模式；T4 独立审计已证明 general agent 能发现自审计 D 段误标的真问题。**对近期子系统做独立审计**：异步统一 A1-A6 / run_batch CPS（exp/run-batch-cps）/ yield 生成器 / generator IbClass / llmexcept / M1-M2 调用收敛——专项查：① sync vs CPS 孪生语义一致；② 快照/双路径"复制完整消费部分"；③ 自审计标注"正确"但实际漂移。用 general agent 独立执行 + 主代理交叉核验。**已执行（2026-08-11，general agent 独立审计 + 主代理核验）**：无 P0；3 确凿 P2 漂移已修复（run_batch 观测 / active_intents 漂移 / _drive 装箱一致）+ generator 兜底 fail-fast + KNOWN_LIMITS §二十四 机制描述修正（commit df1a896）。**遗留疑似项待独立窗口**：① dispatch 路径 `__outputhint_prompt__` 同步 `.call` 嵌套调度器（`_prompt.py:268`）；② `IbClass.receive('__call__')` 捕获主 EC、spawned task 内类构造跨线程改写主 EC 模块名（`ib_class.py:327/273-284`）；③ task 线程内生成器驱动写主线程单写槽（`_core.py:173` "仅主线程"契约 vs task 线程 `_record_current_call_info`）；④ call_intent 短路仅 inline 可达（dispatch/run_batch 不传，AST 无 intent 字段，`_behavior.py:150-162/238-252` 死代码——与 PT-DEBT-24 同源）；⑤ `_get_llmoutput_hint` 与 `_cps` 双实现维护性（`_prompt.py:243-359`） |
| **✅ 已修复（2026-08-12，unsafe-vibe-dev，全量 2235 passed / 1 skipped）** | **PT-DEBT-25/26/27/28 已根治** | 架构健康性 | 按 `_FIX_PROPOSAL_CRITICAL_REVIEW_20260812.md` 修正后方案执行。**PT-DEBT-25（global）**：symbol_resolution_pass 加 visit_IbGlobalStmt + prescan 排除 global 名（镜像 nonlocal），运行时零改动；+6 测试。**PT-DEBT-26（整模块 import）**：档3 写入侧 `_symbol_to_member` 统一 Symbol→MemberSpec/MethodMemberSpec（scheduler.py）+ 档2 `_expression_visitors` 零参数 callable 按 kind 绑定（不再退化 any）；+4 测试。**PT-DEBT-27（异常逃逸）**：`_drive_loop_gen` Waitable yield 加 except Exception 重投递 pending_exception（与值投递对称，TaskCancelled 穿透）；+4 测试。**PT-DEBT-28（ai vtable）**：`_spec.py` 补 get_retry/is_auto_intent_injection_enabled 注册；+3 测试。独立复核三修复全部 PASS（含 TaskCancelled/UnhandledSignal/异常沿 CPS 栈上抛验证）。**遗留**：嵌套包 `import subpkg.util` + `subpkg.util.fn()` 的 INT_INTERNAL_ERROR 为修复前既有缺陷（git stash 实证非本批引入）——中间模块符号以 VariableSymbol(kind=MODULE) 形态经 `import subpkg` 注入，成员访问仍触发 type_ref 缺失；待独立窗口（与 PT-DEBT-26 同族形态问题） |
| **待独立窗口（2026-08-12 真实 LLM 全面压力试用发现，记录不修复）** | **PT-DEBT-25~28 真实 LLM 压力试用缺陷候选**（`tasks_docs/_LLM_TRIAL_20260812/REGISTER.md`） | 架构健康性 | **本任务只试用记录，不修复**（用户 2026-08-12 定案）。**KERNEL-ISSUE-001**（PT-DEBT-25）：`global` 写访问函数内 `RUN_UNDEFINED_VARIABLE`（`bump:counter` 局部 UID），文档 02 §2.6 示例失效，tests 零 `global` 覆盖。**KERNEL-ISSUE-002**（PT-DEBT-26）：整模块 `import mod` + 成员访问 → 编译器 `INT_INTERNAL_ERROR`（`FunctionSymbol/VariableSymbol` 无 `type_ref`），命名导入正常。**KERNEL-ISSUE-003**（PT-DEBT-27）：真实 LLM provider 层失败（超时）`LLMCallError` 逃逸 `try/except`（`ThrownException(error_obj)` 经 worker/Future 回传未被 VM 匹配），MOCK 与手动 raise 均可捕获；文档 04 §4.7/10 §10.3 契约在真实失败路径失效。**KERNEL-ISSUE-004**（PT-DEBT-28）：`ai.get_retry()`/`ai.is_auto_intent_injection_enabled()` 文档化但 `_spec.py` vtable 未注册 → `None` 不可调用。**DOC-ISSUE-001~007**：02_variables 示例缺返回标注 / 9 处 `func __init__` 缺返回标注（04/06/KNOWN_LIMITS/subsystems）/ 14 §14.6 await thread 示例矛盾 / 11 §11.3 stream_call 签名缺失 / howto thread_result `.value` 属性错误 / KNOWN_LIMITS §十二 `SEM_INTENT_STATIC_CALL` 警告未现 / 06 §6.6 `__from_prompt__` 返回 `(bool,实例)` 契约未注明。**BOUNDARY-001~005**：for=to_list 物化（05 §5.8 vs §5.9 张力）/ 生成器 `await chan.recv` 未复现 KNOWN_LIMITS §二十四 错误（文档疑过时）/ 隔离子环境不继承 LLM 配置 / `@!` 在 run_batch 单语句多调用仅首调用生效（文档"一条语句"粒度 vs 实际"一次调用"粒度）/ `probe_model` 误判 reasoning:false 模型。**A1-A5 重验全通过**（意图赋值路径/内建遮蔽/generator.to_list/dispatch 观测/run_batch 观测） |
