...
| **当前阶段（阶段 3 待用户授权）** | **unsafe-vibe-dev 合并取代 main**（`_MAIN_MERGE_PLAN.md`） | 发布/稳定性 | 阶段 2 收尾准备完成（doc-health P1/P2 + pyproject 0.2.0 + examples 真实跑通 11 例 + `_MERGE_READY_REPORT.md`）。**前置已满足（2026-08-12）**：真实 LLM 全面试用重启通过（`_REAL_LLM_TRIAL_REPORT_20260812.md`，合并检测维度已基于修复后代码确认）+ 缺陷修复落地（全量 2270/1）。**阶段 3（`git merge unsafe-vibe-dev → main`）待用户显式授权**（push/合并不在自主范围，禁 push 硬原则） |
...
| **P1（下一 session 主线）** | PT-FEAT-2 Enum 非 str 成员 + 迭代能力 | 易用性+长远 | **评估更新（2026-08-12）：地基已备可正式补全**（`_ENUM_SWITCH_COMPLETION_HANDOFF.md`）。enum 任意类型成员语法已放开；非 str 枚举 LLM 集成缺陷（成员名≠值）+ 迭代/数量/自定义方法缺失。地基成熟（axiom/GenericTypeRegistry/prompt/CPS）。**非设计冻结级变更，可正式实现** |
...
| **待独立窗口（2026-08-12 真实 LLM 全面压力试用遗留）** | **DOC-ISSUE + BOUNDARY 清单**（`tasks_docs/_LLM_TRIAL_20260812/REGISTER.md`） | 架构健康性 | **PT-DEBT-25/26/27/28 已修复（2026-08-12）**（见上方已修复行）。**DOC-ISSUE-001~007**：02_variables 示例缺返回标注 / 9 处 `func __init__` 缺返回标注（04/06/KNOWN_LIMITS/subsystems）/ 14 §14.6 await thread 示例矛盾 / 11 §11.3 stream_call 签名缺失 / howto thread_result `.value` 属性错误 / KNOWN_LIMITS §十二 `SEM_INTENT_STATIC_CALL` 警告未现 / 06 §6.6 `__from_prompt__` 返回 `(bool,实例)` 契约未注明。**BOUNDARY-001~005**：for=to_list 物化 / 生成器 `await chan.recv`（KNOWN_LIMITS §二十四 已修）/ 隔离子环境不继承 LLM 配置 / `@!` 在 run_batch 仅首调用生效 / `probe_model` 误判 reasoning:false。**A1-A5 重验全通过**。**嵌套包 `import subpkg.util` INT_INTERNAL_ERROR**（修复前既有，与 PT-DEBT-26 同族形态问题） |
...
| PT-FEAT-2 | Enum 非 str 成员 + 迭代能力 | **评估更新（2026-08-12）：地基已备，可正式补全**（`_ENUM_SWITCH_COMPLETION_HANDOFF.md`）。enum 现状：任意类型成员（int/float/bool/str）**语法已放开**（KNOWN_LIMITS §二 过时）；str 枚举全能力可用；**非 str 枚举 LLM 集成不工作**（EnumAxiom.from_prompt 返回成员名非值，str 枚举因值==名碰巧工作）；迭代/数量/自定义方法不支持。地基（axiom 体系 / get_method_specs / GenericTypeRegistry / prompt 协议 / CPS 驱动）全部成熟。**建议**：① 非 str 枚举 LLM 集成修复（成员值写入 MemberSpec.metadata + from_prompt 映射）；② 自定义方法；③ 迭代 `for v in Color:` / 数量 `len(Color)`（GeneratorAxiom 先例）；④ KNOWN_LIMITS §二 更新。**下一 session 主线（优先）** |
