...
| **🔴 P0（进行中，2026-08-16 用户指定）** | **协议化内核大重构 + 内核全方位梳理清洁 + 架构级重构**（exp/protocol-kernel 独立分支） | 架构健康性 | **用户拍板：协议化地基重构为主，压力测试/真实试用在其后**。上一 session 因 context 超限中断（恢复交接 `tasks_docs/HANDOFF_PROTOCOL_KERNEL_RECOVERY.md`），正式任务控制文档未及时同步属交接割裂，已补齐登记。**已完成 39 commits**：协议内核地基 / 用户协议语法 / 泛型约束与泛型函数 / 泛型协议 / retroactive（声明式）/ 普通函数与 LLM 函数全链路统一（删独立 IbLLMFunction）/ 协议注册表替换硬编码能力（多站点）。**待做**：剩余硬编码能力替换、IbLLMFunctionDef 残留清理、Prompt 装配散落统一、序列化/动态宿主/插件体系类型分支清理、retroactive 推进到可补方法、文档同步（LANGUAGE_DESIGN_EVOLUTION）。每阶段全量 pytest 零回归门。**完成后**：最新代码上开启全方位真实 LLM + 真实 IBCI 试用（trials 套件）确认重构影响并启动新压力测试（原 T08 压力试用主线顺延至此之后）。 |
