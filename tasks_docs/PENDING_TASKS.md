...
| 优先级 | 任务 | 维度 | 说明 |
|--------|------|------|------|
| **🟡 待修（2026-08-14 T05 批判试用发现，独立窗口）** | **KERNEL_ISSUE-CROSSMOD-THREAD-1：被 import 模块用户类在线程 worker 内不可用** | 架构健康性 | 触发 `trials/T05_critical_stress/cases/D1-10/`。主上下文 `geo.Box(5).get()`=405；线程 worker 内同调用抛 `ThreadFailed`（底层 `Symbol UID missing for name 'v'`）。入口模块类 worker 内正常；**base（main 分支）同现=既有缺陷**，非 module 化引入。根因方向：线程 worker task 上下文未带出方法体所属模块的 artifact/符号侧表上下文。与 14_concurrency §14.1"共享只读类型定义"承诺矛盾。**P1**，待独立窗口（含根因分析+修复+回归）。 |
| **🟡 待修（2026-08-14 T05 批判试用发现，独立窗口）** | **KERNEL_ISSUE-OPTIONAL-ISNONE-1：`Optional[T] a = None; a is None` 返回 False** | 语义正确性 | 触发 `trials/T05_critical_stress/cases/D2-03.ibci`。Optional None 被 `IbOptional(is_some=False)` 包装，`is` 对 None 分支 `isinstance(left, IbNone)`（leaf.py）→ False；`a == None` True、`any b = None; b is None` True。文档 03_operators.md:70 声称 `x is None 判 None`。**P2**，与 DOC_ISSUE-5 关联。 |
| **🟡 待修（2026-08-14 T05 文档核验，独立窗口）** | **DOC_ISSUE-1~23 文档批量** | 文档健康 | T05 全量文档核验产出 23 条（P1×4/P2×11/P3×8）：MOCK 值语义（STR 多词 token/BOOL 大写判定）、lambda 语句体示例、cast_to 示例、Optional `is None`/`is_none` 缺失、KNOWN_LIMITS §七/§八/§十三 三条自相矛盾、15_diagnostics 8 个死码、快照篡改警告、线程跨模块承诺、TESTONLY 遗留术语、`find_last` 索引、`set_mock_mode` 单向性等。完整清单见 `tasks_docs/trials/T05_critical_stress/REGISTER.md`。**P1-P3**，文档修复批次（doc-governance Phase）。 |
