...
| DEBT（技术债） | 4 | 1 | 0 | 架构缺陷与清理项（PT-DEBT-4/31 已实证核实 done） |
...
- **状态**：active（评估→调研→修复；用户 2026-08-20 指定：**放置在真实 LLM 全面试用
  （阶段 C）之前**）｜**域**：DEBT｜**优先级**：P1
...
- **成因**：值语义（可变 list/dict 的引用 vs 拷贝）、绑定语义（赋值 = 重绑定 vs 就地突变）、
  ...
  缺统一建模与文档契约。
- **当前理解**：需先**评估**现状语义面（引用 vs 拷贝的判定规则、赋值语义、参数传递语义、
  闭包捕获语义），再**调研**主流语言对照（Python/C++/Rust/Java 等），最后**修复**语义混乱
  并补齐语法与说明书（docs/）权威契约；该隐患在修复前约束着真实试用（阶段 C）的评估可信度，
  故排在试用之前。
- **关联**：PENDING §九 设计决策（运行时值 `type_ref` 保持基础 spec——可变 list/dict 不固有
  ...
  docs/ 值语义相关章节（`03_type_system.md` 等）。
