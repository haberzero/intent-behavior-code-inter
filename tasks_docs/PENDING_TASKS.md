...
>
> **最后更新**：2026-08-03（新主线确立：IBCI 运行时多线程 + 统一通信机制 + 内省/控制，见 NEXT_STEPS.md；新增 PT-MT-* 系列规划任务）

---

## 〇、IBCI 运行时多线程 + 通信机制（新主线规划，见 NEXT_STEPS.md）

> 用户裁定（2026-08-03）确立下一主线。核心：一等通信机制（Channel/Signal/Slot）+ 内省 + 控制 + 多 VM 实例 + 编译器改造。接受破坏性改造，架构正确性 > 维持现状。
>
> 本系列为**待设计**的规划任务（详细设计文档先于实现）。完成顺序：设计文档 → 编译器地基 → 通信内核 → 内省/控制 → 流式/并行 → 多 VM 实例 → 测试。

| 编号 | 标题 | 目标 | 状态 |
|------|------|------|------|
| PT-MT-1 | 详细设计文档 | 完整架构/AST 变更/接口/并发正确性/测试策略，先经用户审阅 | **已完成 2026-08-03（待用户审阅）**，见 `tasks_docs/THREADING_DESIGN_DETAIL.md` |
| PT-MT-2 | 编译器地基 | 新 AST 节点（spawn/join/task/chan/signal/slot）+ parser + 4 阶段语义 + dispatch + 序列化（任务为运行时瞬态） | 待做（待 PT-MT-1 审阅通过） |
