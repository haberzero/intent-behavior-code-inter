...
| PT-DEBT-17 | `ai.run_batch` 同步阻塞路径（统一执行模型冲突） | **2026-08-10 彻查确认（三层设计不一致，非死代码误判）**：`run_batch`（`_behavior.py:339`）是 `ai.run_batch` 语言特性（`ibci_ai/core.py:311` → `executor.run_batch`），**真消费**。但作为语言特性走同步阻塞路径：**①** 主线程调用同步 `_prepare_behavior_call`（:369）→ `_evaluate_segments` `vm.run` 重入调度循环（`_prompt.py:153`，与 A2 同款、独立位置，`_HEALTH_AUDIT_PLAN` 原标记"仅 docstring 引用"为误判）；**②** 主线程 `fut.result()`（:383）同步阻塞等全部 LLM 完成（实测 415ms/3×SLEEP:300，多任务并发拖住调度线程，违反"阻塞即挂起"）；**③** 同模块 `stream_call` 返回 `IbStreamHandle`（Waitable，auto-yield 挂起）而 `run_batch` 返回 `List`（非 Waitable，主线程硬阻塞）——并发范式割裂（design-philosophy §三/§四）。**修复方向**：`run_batch` CPS 化（主线程预求值改 yield + 多 Future 聚合 Waitable 让出调度线程）；`ai.run_batch` 语言层契约评估（vtable return_type）。涉及对外契约（vtable），独立窗口/需裁决 |
