...
| **🟡 待修（2026-08-14 T06 试用发现，pre-existing，独立窗口）** | **KERNEL_ISSUE-CROSSMOD-LLM-1：跨模块用户类作行为表达式 LLM 输出目标失败** | 编译期正确性 | 触发 `trials/T06_class_identity/cases/D3-02/`、`D2-05/`。`geo.Counter c = @~...~` 运行时抛 `__call__ on None`。**根因（编译期实证）**：行为表达式赋给模块限定类型（annotation 为 `IbAttribute` 点号限定）时，编译器未把目标类型绑定到行为节点 node_to_type（`_statement_visitors.py:110-117` bind_type 对 IbName 目标生效，`_resolve_type` 对 IbAttribute 注解未解析出 geo.Counter spec）→ node_to_type='behavior' → LLM parse 链 type_hint='behavior' → 运行时崩溃。**对照**：入口类 `Point p = @~...~`（IbName 注解）正确。**base 同现=pre-existing**，非 S2 引入。比 KNOWN_LIMITS §10.2"graceful 退化"更严重（运行时崩溃）。**P1**，独立窗口（`_resolve_type` 支持 IbAttribute 点号限定注解 + behavior 绑定补全，判别性回归 `geo.Counter c = @~...~` 与入口类对称）。 |
| **🟡 待修（2026-08-14 T05 批判试用发现，独立窗口）** | **KERNEL_ISSUE-OPTIONAL-ISNONE-1：`Optional[T] a = None; a is None` 返回 False** | 语义正确性 | 触发 `trials/T05_critical_stress/cases/D2-03.ibci`。Optional None 被 `IbOptional(is_some=False)` 包装，`is` 对 None 分支 `isinstance(left, IbNone)`（leaf.py）→ False；`a == None` True、`any b = None; b is None` True；`is_none()` 文档有实现缺失。文档 03_operators.md:70 声称 `x is None 判 None`。**粗略根因已定位（见 `_HANDOFF_T05_ISSUES.md` §2.2）**。**P2**，与 DOC_ISSUE-5/-6 合并处置（语义对齐 + 补 `is_none` + 文档）。 |
| **🟡 待修（2026-08-14 T05 文档核验，独立窗口）** | **DOC_ISSUE-1~23 文档批量（含 4 项代码联动：mock STR/BOOL 值语义 / 8 幽灵诊断码+快照警告 / `is_none()` 缺失（与 KI-2 合并）/ `set_mock_mode` 单向）** | 文档健康 | T05 全量文档核验产出 23 条（P1×4/P2×11/P3×8）：MOCK 值语义（STR 多词 token/BOOL 大写判定）、lambda 语句体示例、cast_to 示例、Optional `is None`/`is_none` 缺失、KNOWN_LIMITS §七/§八/§十三 三条自相矛盾、15_diagnostics 8 个死码、快照篡改警告、线程跨模块承诺、TESTONLY 遗留术语、`find_last` 索引、`set_mock_mode` 单向性等。**代码联动项已在 `_HANDOFF_T05_ISSUES.md` §二 单独列出（优先于纯文档项）**；完整清单见 `tasks_docs/trials/T05_critical_stress/REGISTER.md` + `_HANDOFF_T05_ISSUES.md` §三。**P1-P3**，doc-governance Phase（代码项先行）。 |
