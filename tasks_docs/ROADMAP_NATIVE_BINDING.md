...
### F6 —（下一条主线）llm 机制重构为可调用的 llm 类
- **方向（用户 2026-08-18 定）**：彻底抛弃"llm 函数"概念，重新设计为**可调用的 llm 类**
  （面向对象形态承载 LLM 调用 / 意图 / llmexcept 等语义）。
- **状态**：方向已定，**调研与需求确定留给下一 session**（盘点现 llm 函数语义面 + 设计
  llm 类形态 + 评估破坏面/迁移）。实现前须经 code-workflow + 全量 pytest 破坏面评估。
- **验证门**：调研/设计产出 + 全量 pytest 零回归（实现阶段）。
