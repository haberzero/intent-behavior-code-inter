...
| # | 位置 | 观察 | 潜在风险 | 来源 | 状态 |
|---|---|---|---|---|---|
| A1 | `ibci_isys/core.py`（已改）/ `ibci_idbg/core.py` | 防御性内省（可选项注入、调试工具、安全默认）——**isys 已契约直访（2026-08-05 修正）；idbg 消费者 hasattr 已清，防御性访问仍存** | 协议演进时这些探测可能变恒真/恒假，或漏掉新能力 | 质量维护·维持现状 | 部分已改（2026-08-05） |
