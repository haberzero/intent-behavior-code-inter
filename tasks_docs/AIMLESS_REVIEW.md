...
| # | 位置 | 观察 | 潜在风险 | 来源 | 状态 |
|---|---|---|---|---|---|
| A1 | `ibci_isys/core.py`（已改）/ `ibci_idbg/core.py` | 防御性内省（可选项注入、调试工具、安全默认）——**isys 已契约直访（2026-08-05 修正）；idbg 防御性访问已清理（2026-08-06 OBSERVABILITY 2C：fields() 改 isinstance(IbObject) 协议直访，show_intents 单一权威源）** | 协议演进时这些探测可能变恒真/恒假，或漏掉新能力 | 质量维护·已解决 | 已全部清理（2026-08-06） |
...
| A3 | `llm_parsing_strategy.py` / `_prompt.py` 宽 except | 解析失败→`uncertain`/降级链（LLM 输出不确定→重试语义）——**已全部收敛（2026-08-09 事实回顾）**：`_obj_to_prompt_str`（_prompt.py:42）窄化 AttributeError（协议缺失回退）+ 用户实现体内真实 bug fail-fast；`_obj_to_payload`（_prompt.py:86/91）窄化 AttributeError + 宽 Exception 经 `kernel_diagnostic`（KDIAG_PROTOCOL_PAYLOAD_PROMPT_FALLBACK）上报非静默 | 用户 `__from_prompt__`/`__to_prompt__` 内真异常已被 fail-fast/诊断上报覆盖 | 质量维护·已解决 | **已解决**（2026-08-09 复核确认） |
