...
1. **C1 embedding 立项边界**：与 PT-SEALED-1 media Phase 4 封存的关系（ref 主张
   embedding 是语义内容缝基础能力、非媒体，请求独立立项）。
2. **PT-DECIDE-2 解封重估**：新端点实证后思考抑制缺口收窄，是否解封收窄范围。
3. **批 0 先行确认**：阶段 C 残留项清场是否作为阶段 E 开场；周期质量维护恢复时机随排布定。
4. **ref 来源项目协同方式**：需求单来自外部灰盒自动机工作区（e01-e17 / trial2 实证）；
   该项目是否切换到主线 IBCI 消费新能力、验收以何为准。
