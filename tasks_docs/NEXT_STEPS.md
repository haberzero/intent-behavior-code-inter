...
## 下一任务：orchestrator 注入双通道 + 生产死参数链修复（组 6，最高优先级）

> **来源**：组 1 修复后自我质询中，`set_orchestrator` 同步分支曾被误判为"死代码"；用户指出"误判可能暴露设计不一致"。完整时序追踪确认：**该分支是功能必需代码，但掩盖了 orchestrator 注入链的构造死参数缺陷**（双通道）。完整分析与候选方案见 `tasks_docs/REFLECTION_SCAN.md` §三-A。

- **问题本质**：生产路径的 orchestrator 构造注入链恒失效（`HostService`/`Interpreter`/`ServiceContextImpl` 构造参数恒收 None），真实注入完全依赖 `set_orchestrator` 事后同步 → 双通道 + 死参数链。
- **待裁决**：方案 A（删除构造参数，收敛单一注入，推荐）vs 方案 C（最小修补，双通道残留）。裁决后开工。
- **验证**：全量 pytest 零回归 + 残留扫描。

---

## 后续任务：反射规避架构缺陷修复（组 2+，在组 6 之后）
...
- **核心结论**：未发现恶意穿透案例；核心模式为"协议声明滞后于实现 + 接口损坏被迫穿透 + 历史手势残留"，与 PT-HEALTH-1 同族。共 5 个缺陷组 + 合法反射保留清单 + 死代码/双通道清理清单 + 冗余探测简化清单（组 1 已完成）。
- **修复顺序**：组 2（插件公开接口 + dir() 白名单）→ 组 3/5（异常探测/静默兜底）→ 组 4（序列化私有穿透）→ 死代码清理 → 冗余简化。
- **上报前置**：组 2 涉及契约变更/跨子系统取舍，修复前须向用户确认方向（见 REFLECTION_SCAN.md §十二）。
