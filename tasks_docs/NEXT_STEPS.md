...
- **当前主线（架构健康性优先，用户 2026-08-08 定案）**：**异步地基遗留妥协根治**（统一执行模型闭环）——
  ...
  PT-DEBT-14（F2 slot.update + F3 prompt hint CPS 化）已完成（2026-08-08，全量 2071/1）**；
  **下一步 PT-DEBT-15（M1 .call 收敛 / M2 驱动去重 / M3 prompt 单源 / M4 LLM 真挂起，中严重度）**。
  实施计划见 `tasks_docs/_ASYNC_UNIFY.md`（F1→B1→F2/F3→M1-M4）。登记 PT-DEBT-12/13/14/15。
