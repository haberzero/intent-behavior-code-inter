...
>
 ...
 > Phase 0 实证裁定 + B1 ✅[共享合成] + B2 ✅[工具 4 契约源自举：契约源 4 件 +
 > bootstrap 阶段 + 实现重打包 + 字面量真删除，全量 3963/1 零回归]；B3[net]实施期实证
 > 取消[net headers 默认参数面超出 bind 表达力 + per-engine 状态 → 维持宿主侧字面量]；
 > 批次计划与实施期裁定记录 = tasks_docs/_p6_selfbootstrap_design.md；下一批 = B4 文档收敛）；P3 D-3.3[紧迫性下调]。
>
...
> 开工**（bind 表达内核契约；B1 ✅ 共享合成 + B2 ✅ 工具 4 契约源自举[全量 3963/1
> 零回归] + B3[net]实施期实证取消[headers 默认参数面超出 bind 表达力 → net 维持
> 宿主侧字面量]；设计文档 `tasks_docs/_p6_selfbootstrap_design.md`；下一批 = B4
> 文档收敛 + 里程碑收束 ff——P1-P5 已全部完成收束，git 承载）。硬约束 = 9 项 VM 设计不变量
