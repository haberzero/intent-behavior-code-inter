...
>
> **最后更新**：2026-08-16（**协议化大重构完成并合并；下一主线：内核协议化收尾与双轨收敛**；
> 交接见 `tasks_docs/_HANDOFF_KERNEL_PROTOCOLIZATION.md`）
>
> **🔴 当前主线（2026-08-16 用户指定，待下一智能体接手）**：
> **内核协议化收尾与双轨收敛（体系化重构）**——把协议化/单一权威标准推向 OOP 侧与
> 编译期-运行期边界。**阶段 A**（P0）：OOP 侧协议化——dunder 协议注册表化，消除
> receive 字符串消息分派与 `message ==` 硬编码分支（base.py:51/71/87、
> ib_class.py:603/620/626）、`_default_getattr` 字符串兜底、`_impl_cls` 按名解析。
> **阶段 B**（P0）：双轨收敛——方法符号 spec self 形态编译期统一、特化身份结构化
> （`"Box[int]"` 字符串注册键/boxed 名桥归零）、成员单一权威、auto-init 声明化
> （数量校验三处归一）、node_to_symbol 绑定双义根治。**阶段 C**（P0 关联）：公理
> 声明协议化（has_*_cap 布尔字段 → 协议注册表声明驱动）。**阶段 D**（P1）：KNOWN_LIMITS
> 26 节三分类处置（§4 归类初稿）。**阶段 E**（P2）：宣发前置（性能基准/工具链/文档旅程）。
> 每阶段验收门：全量 pytest 零回归 + 独立复核 + 真实 LLM 试用复跑（分类逐例一致）+
> 文档同步。完整隐患清单/断裂专项/启动指引见交接文档。
>
> **✅ 已完成（2026-08-16）**：**exp/protocol-kernel 直接合并 unsafe-vibe-dev**
> （用户显式授权，fast-forward 1b83a698..fbda2098，79 文件 +4216/-784）。
> 合并门：全量 pytest 2912/1 零回归 + 116 例真实 LLM 复跑分类与基线逐例一致
> （含方法 spec 根治后 `__init__` 契约校验生效面无回归）。禁 push 原则仍有效
> （本地合并，未 push）。
>
