...
> **新 session 开工前必读**：`tasks_docs/WORK_HABITS.md`（用户工作习惯与质询模式）。
>
> **最后更新**：2026-08-02（反射排查长 session：组 2/3/5 完成，组 4 下一）
...
## 下一任务：反射规避架构缺陷修复（组 4：序列化/私有属性穿透）

> **来源**：处理 PT-HEALTH-1 时用户裁定——代码中可能存在更多"用反射/探测规避架构设计缺陷"的代码，需全仓找出并**确认背后是否确为架构缺陷**（若是则修架构根因，而非继续用反射绕开）。
>
> **已完成（2026-08-02）**：组 1（LLM 协议并入）、组 6（orchestrator 注入收敛）、组 7（组 1 深度复核）、组 2（能力注册/模块导入体系）、组 3/5（异常探测/静默兜底）。

- **扫描与 6-subagent 复核已完成（2026-08-02），结论已收敛**。完整记录 + 修复检查清单 + 需用户确认项见 `tasks_docs/REFLECTION_SCAN.md`（单一事实来源，本文档不再重复细节）。
- **核心结论**：未发现恶意穿透案例；核心模式为"协议声明滞后于实现 + 接口损坏被迫穿透 + 历史手势残留 + 隐式契约/字符串约定"，与 PT-HEALTH-1 同族。共 8 个缺陷组（组 8 为非 hasattr 反射面重复核，见 §三-C）。
- **修复顺序**：组 4（序列化私有穿透，含 5.4）→ 死代码清理 → 冗余简化 → 组 8 项（异常错误码/能力名常量/签名静默降级/`__getattr__` 透传安全矛盾）。
- **上报前置**：组 4 涉及 Scope/Context 接口新增（UID 枚举、`bind_symbol_by_uid`、loop 栈快照），修复前须向用户确认方向（见 REFLECTION_SCAN.md §十二）。
