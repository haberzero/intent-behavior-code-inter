...
> 全量 4326/0/1。
> **✅ R3-C 测试体系重构全部推进完毕（2026-09-11）**：五层（behavior/contracts/
> host/前端/内核内部 cargo test）组建完成；19 白盒文件迁移映射全部执行（删除 +
> 行为/宿主承接 + 归类保留）；内核单测 6/0。
> **✅ R4 HOST-EXT + pip / R5 计算编排协议 / R6 Rust 插件协议（2026-09-11）**：
> register_host_extension + compute_engine（NumpyEngine 编排）+ ibci-sdk 插件 ABI。
> **✅ 阶段 D（⑦ 终点）全部落地（2026-09-11，D1-D7）**：角移除（intrinsic/
> tuple/kb_vec）→ 宿主桥接 + 6 模块路由 Rust（compute_engine/plugins/fs/
> world_model/ai）→ 数据面差分退场（语料契约权威化）→ 双码族单源 + UID 规范
> 裁决 → 参考角色退役定论 → **前端差分 + 语义转录退役（-4425 行）**——⑦ 终点
> 架构达成：**Rust = 唯一语义执行内核（反序列化 + 解释 + kb + 插件 + 任务池）；
> Python = 编译器 + LLM/宿主执行层；无转录层**。async/ihost = 宿主执行层保留
> （职责重定位）。当前下一步 = 周期维护（Tier B 扫描 + 文档对账 + 内核健康持续
> 关注）；PENDING 无重估触发阻塞项。
>
> **测试基线（唯一锚点）**：`.venv/bin/python -m pytest tests/`（唯一权威命令；addopts 已含 `-q`，
> 勿显式再加——双 `-q` 会隐藏计数行）；末次全量 **4268 passed / 0 failed / 1 skipped**（2026-09-11
> 实跑 ~124s，c5b63344[阶段 D7：前端差分 + 语义转录退役]——差分机制全部退场后基线；
> 数字以实跑为准，不冻结）。
...
1. **主线 = IBCI 原生数据库（世界模型知识图谱）+ 自指性收束 + 测试进程内化**（见上"当前
   ...
   → 第四批 增量 1 quoted + meta 模块原生面[去 Host 化：Quoted{source} + MetaFn 变体
    ...
    → 第四批 增量 2a KB 语料面 Rust 原生数据模型[kb.rs：KbState 治理词表 + append-only
       ...
       [kb_corpus 桥接版]；错误面 = 跨切面后续；41 成员面 22 已载] ✅
    → 第四批 增量 2b KB 查询/审计/对比/展开/传递面[kb.rs +13 方法：word/relation/
       world 查询[未注册 None] + get_fact/facts/fact_len/all_in_world/source/
       history_fact + amend_fact[o 版本化 + by_triple 切换] + retract[墓碑 +
       active 索引移除] + same_word/compare/expand[纯派生] + transitive[BFS 闭包
       确定性发现序]；25 行合成探针无桥接全对齐一次通过；41 成员面 22 已载] ✅
       确定性发现序]；25 行合成探针无桥接全对齐一次通过；41 成员面 22 已载] ✅
    → 第四批 增量 2c vector 值 + KB 嵌入面[IbValue::Vector 不可变值[截断摘要
       显示面 + format_g6 = Python %.6g 等价助手] + vec intrinsic + dim/dot/
    ...
    → 第四批 增量 3b CPS 覆盖差收缩 31→36[5 节点移植：global/nonlocal[运行时
       ...
       interpreter 五面全载；3 探针 + switch 源 full_artifact 五池全等价] ✅
    → 第四批 增量 3c 声明面移植[TYPE x = v / auto x = v / 泛型 list[int] xs = v
    ...
    → 第四批 增量 3d 声明面剩余形态 + 函数值一等化[fn f = g 可调用声明[值推导
       ...
       顺序修正]；2 探针数据面 + 5 池 + 2 侧表内容归一全等价] ✅
    → 第四批 增量 3e 错误面统一[异常传播机制——Thrown 值传播全签名
       Result 线程化 + IbValue::Error 异常对象[8 类构造器 + `<class>:
