...
>
 ...
 > 零 __reduce__/__setstate__ 定义）+ 缓存目录 0700/payload 0600 + 篡改拒绝 stderr 可观测 + 命中/未命中
  ...
  > commit 9eb638ca）。**全量 3938/1 零回归**。
 >
 > **P6 内核自举（bind 表达内核契约）✅ 完成 + 里程碑收束（2026-09-09）**：Phase 0 实证
 > 裁定（bind 化范围 = 工具 4；kernel 5+fs 维持宿主侧；F5 时序矛盾裁定精化）+ B1 ✅ 共享合成
 > 函数提取（host_spec_synthesis 单一权威源）+ B2 ✅ 工具 4 契约源自举（contracts/ 4 件 IBCI
 > bind 声明 = 契约单一权威源 + kernel_contracts bootstrap[零新运行期机制] + 实现重打包
 > [类实例→模块级函数] + 4 字面量真删除 + kernel_version 递增 + 判别套件 8 例；全量 3963/1
 > 零回归）+ B3[net]实施期实证取消（net 8 方法 headers 默认参数面 has_default 超出 bind
 > 表达力[F3-0 裁定未推翻] + per-engine 可变状态双重边界 → net 维持宿主侧字面量；远期项
...
> + 反射（高工作量、结构面）→ P7 档 B 进程级隔离（高风险→隔离分支）**。**P6 内核自举 ✅ 完成 +
> 里程碑收束（2026-09-09）**（bind 表达内核契约：B1 ✅ 共享合成 + B2 ✅ 工具 4 契约源自举[全量
> 3963/1 零回归] + B3[net]实施期实证取消[headers 默认参数面超出 bind 表达力 → net 维持宿主侧
> 字面量] + B4 ✅ 文档收敛；详见上方 P6 块，git 承载）。**当前 P0 = P7 档 B 进程级隔离 + 反射
> （高风险→独立隔离分支 100% 授权；P3 D-3.3 可交错）**。硬约束 = 9 项 VM 设计不变量
>
