...
> 详见 `tasks_docs/COMPLETED.md` 2026-05-27 锚点。以下各节发现处已标注关闭状态。
...
**测试基线**：`pytest` 全跑 **832 passed, 2 skipped in 8.31s**，与 `tasks_docs/COMPLETED.md` 记载完全一致。两个 skip（INV-LAMBDA-3、INV-SCOPE-1）是已记录的设计限制。
...
依据 `docs/SYNTAX_REFERENCE.md`（1401 行）逐项核对：
...
| 文件 | 用途 |
|---|---|
| `tasks_docs/NEXT_STEPS.md` | 当前最紧要下一步 + 维护守则 |
| `tasks_docs/COMPLETED.md` | 完成时间线；最近：Phase 2 payload_prompt (2026-05-27) |
| `docs/KNOWN_LIMITS.md` | 590 行；§26 在本次审计当日新增 |
| `docs/SYNTAX_REFERENCE.md` | 1401 行，用户侧语法权威参考 |
| `tasks_docs/PENDING_TASKS.md` | 已阻塞/等待中的未来规划 |
