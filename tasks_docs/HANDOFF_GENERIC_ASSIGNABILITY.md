# 交接：泛型赋值检查缺失 + type() 内省不对称（✅ 两缺陷均已修复 2026-08-13）

> **状态更新（2026-08-13）**：本文件交接的两处缺陷**已全部根治**（用户裁定两缺陷均修复，
> 架构统一性/长期收益/代码质量优先）。缺陷一 `80294a64`、缺陷二 `447ad35c` + 独立复核整改
> `7a15c1b9`（unsafe-vibe-dev），全量 2559 passed / 1 skipped。设计冻结
> `tasks_docs/_code_generic_type_identity.md`；修复记录见 WORKLOG。
> 本文件保留作为根因分析/修复方案的决策档案（不复述正文，供追溯）。

---

## 一、缺陷一：内置泛型赋值类型检查系统性缺失泛型实参校验（P1，已修复 ✅）

**修复**：`is_assignable` 在 axiom 兼容前插入同家族结构化实参比较（`_generic_spec_args` +
`_generic_family_compatible`），10/11 类 `X[int]→X[str]` 编译期拦截；协变/子类型兼容保留；
独立复核整改 dict 协变假拒绝 + 跨家族（behavior→fn_callable）实参校验。判别性回归 23 项 + 触发用例 G6-01（GUARD）。
...
## 二、缺陷二（升格）：内建泛型运行时值层类型擦除 = 初期设计缺陷/架构妥协（已修复 ✅）

**修复**：特化 spec 水化为运行时特化类四层（ArtifactLoader 预创建 + `_specialize` 内置泛型
下标水化 + 编译期容器字面量 bind_type + 值层分派沿 spec 基名统一），`type(list[int]值)=list[int]`，
运行时值层可区分 `list[int]`/`list[str]`，深克隆/同引擎序列化 round-trip 保真。判别性回归
12 e2e + 4 runtime 白盒 + 触发用例 G6-02（PASS）。**已确认边界**（`_code_generic_type_identity.md`
§2.6 待增量）：函数返回/实参/下标赋值字面量、嵌套内层、切片、Optional 值身份、跨引擎反序列化。
