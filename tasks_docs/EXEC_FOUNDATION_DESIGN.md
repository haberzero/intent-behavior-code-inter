...
- **架构要点（生成器体必须为单可恢复驱动）**：`yield` 可在循环/条件内，挂起须保留整个帧状态
  （循环位置、局部变量）→ 生成器体不能"逐语句 run()"，必须由单一可恢复驱动生成器承载（同
  `_drive_loop_gen` 模型），在 `yield` 点暂停交付值、迭代恢复。驱动需同时处理体内 `await`
  （waitable 挂起）与语言级 `yield`（交付值）。
- **2026-08-07 阶段 2 界定**：D-08（无 async 关键字，任意函数可 await）已落地验证（test_await_expr
  TestAwaitAnywhere +2）。`yield` 惰性生成器为**大型独立语言特性**，列为阶段 2 剩余工作项
  （`NEXT_STEPS`），在统一清理（阶段 3）与诊断（阶段 4）之后择机专注实现——避免在本轮
  仓促实现半成品。
