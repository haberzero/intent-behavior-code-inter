...
| A2 | **PT-DEBT-34 变量语义建模显式化**（用户 2026-08-20 新增，置顶） | 变量**引用/拷贝/赋值/传递**值语义建模显式化——评估结论：运行时模型一致、非大重构；四步：调研对照→文档权威契约（值语义章节+教学段+修 KNOWN_LIMITS §五.2 漂移）→判别测试→运行时审计+小修；工作量中等（约 4-6 窗口） | 用户裁定；语义地基级隐患；排在真实试用前 | 文档权威契约 + 判别测试零回归 + 审计发散点清零 |
