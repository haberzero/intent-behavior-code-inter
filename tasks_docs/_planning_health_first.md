...
| A2 | ~~PT-DEBT-34 变量语义建模显式化~~ ✅（2026-08-20 完成） | ~~变量**引用/拷贝/赋值/传递**值语义建模显式化——评估结论：运行时模型一致、非大重构；四步：调研对照→文档权威契约（值语义章节+教学段+修 KNOWN_LIMITS §五.2 漂移）→判别测试→运行时审计+小修~~ | 用户裁定；语义地基级隐患；排在真实试用前 | 完成：02_variables §2.8/05_functions §5.10 权威契约 + 判别 +11 + 审计（容器 == 身份比较文档化）；全量 3100 零回归 |
| A3 | ~~PT-DEBT-29 生成器消费协作化~~ ✅（2026-08-20 完成） | ~~`IbGenerator.generic_next` 对 Waitable 阻塞等待 → Waitable 感知挂起（让出型消费，与 CPS 执行模型同构）~~ | KNOWN_LIMITS §二十四；消除同步阻塞死锁风险 | 完成：全消费面协作让出 + 判别 +6 + §二十四移除重编号；全量 3112 零回归 |
| A4 | ~~PT-DEBT-30 + PT-DEBT-33 类型边界闭合~~ ✅（2026-08-20 完成） | ~~`yield from` 序列委托编译期生成器/序列区分；KNOWN_LIMITS §十 dict 键下标校验 / 中置星计数偏移 / 跨引擎封印~~ | 类型地基边界 | 完成：yield from 收紧 + dict 键校验 + 星偏移修正 + 跨引擎封印优雅回退；判别 +11；全量 3123 零回归 |
| A5 | ~~Tier C 专项审计~~ ✅（2026-08-20 完成） | ~~C 类异味 ~25 处需人工判定 + 静默降级补诊断复核（leaf/runtime_serializer/artifact_loader 尽力而为回退）+ for+if 深嵌套可读性（PT-AUDIT-2 收窄项）~~ | 收敛期审计 C 类清单；PT-AUDIT-2 前提复核 | 完成：113 处 hasattr 分类 + 6 阶段零回归 + contract_validator 恢复 + KDIAG 诊断；全量 3133 零回归 |
| A6 | ⏸ PT-AUDIT-1/3 周期复核 + quality-maintenance Tier B（用户 2026-08-20 裁定推迟到真实试用后） | 代码异味核对周期回顾；R 系列复核清单；Tier B 阶段边界批量巡检 | 周期审计；长期质量维护 | 推迟：真实试用（阶段 C）后恢复 |

## 三、阶段 B · 功能稳健与对外能力（✅ 全部完成 2026-08-20，B1-B6 落地，当前 P0 前移阶段 C）

| # | 任务 | 内容 | 维度 |
|---|---|---|---|
| B1 | ~~PT-TEST-2 覆盖缺口补测~~ ✅ | COVERAGE_MATRIX 缺口全收敛（判别 +17，全量 3156 零回归） | 功能稳健 |
| B2 | ~~PT-DECIDE-3 项②④~~ ✅ | 不扩展 validate_prompt 至内置 + impl 内置解析方法编译期拒绝 + to_prompt_str KDIAG 可观测（判别 +4，3160） | 语义完整性 |
| B3 | ~~PT-DEBT-36~~ ✅ | intent_context 方法族收敛 + _is_impl_method 排除元类伪影（bool\| 修复）+ _verify_axiom_bindings 契约校验（判别 +13，3173） | 架构/契约 |
| B4 | ~~PT-DEBT-35~~ ✅ | `_ctx` 契约单一权威 get_intent_ctx/set_intent_ctx（判别 +9，3182） | 架构/契约 |
| B5 | ~~PT-DOC-3P2~~ ✅ | 新增 use_isolation + orchestrate_llm_calls 2 篇 howto + 交叉引用（纯文档） | 对外可用性 |
| B6 | ~~PT-FEAT-5 CI/CD 可靠化~~ ✅ | 四层可靠性设计 + ci.yml 分层就绪（保持 workflow_dispatch）+ scripts/ci_local.sh 本地复现；远程启用待用户授权 | 工程稳健/对外 |
