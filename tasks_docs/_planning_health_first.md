...
## 三、阶段 B · 功能稳健与对外能力（当前 P0，2026-08-20 阶段 A 一次性攻坚完成后前移）
...
| B3 | **PT-DEBT-36 intent_context 方法族结构重构 + axiom 能力契约校验**（2026-08-20 从推迟移入 B） | intent_context OOP 方法族层边界重构 + bootstrap 契约校验 axiom 声明能力绑定 | 架构/契约（与 B2 协议主题衔接） |
| B4 | **PT-DEBT-35 `_ctx` 内部契约完整形式化**（2026-08-20 从推迟移入 B） | 定义 `_ctx` 契约单一权威（类型/协议判别），收敛 ~20 处访问，消除字段探测双轨 | 架构/契约（与 B3 同族） |
| B5 | **PT-DOC-3P2 how-to 读者旅程补齐**（提前至 B5） | Reference→How-to 读者旅程断裂；生成器/并发/llmexcept/隔离等场景操作指南 | 对外可用性 |
| B6 | **PT-FEAT-5 CI/CD 可靠化** | CI/CD 可靠化/实用化设计后重新启用（真实 LLM e2e / 跨平台 / 发布产物）；前三项已落地 | 工程稳健/对外 |
