...
### 2.3 双内核协议（迁移机制——**临时**，非永久终点；见 §2.6 终点裁定）

> **定位演进（2026-09-10 用户裁定）**：双内核 + 差分 harness 是**迁移期的安全网**
> （Python 内核 = 参考基准 + 调试路径，Rust 内核 = 待证明的生产内核）。**终点 =
> 全量 Rust 化**（见 §2.6）——证明绝大部分核心逻辑可 Rust 化后，废弃 Python 双
> 通道/对比，全量转向 Rust。迁移期双内核协议如下：

- **Python 内核 = 一等实验内核 + 参考基准**：迁移期 `kernel="py"` 继续 Python 侧
  灵活修改（热迭代、原生 traceback 调试）+ 作差分 harness 的参考基准
- **Rust 内核 = 生产快路径（待证明）**：显式启用（配置/CLI 旗标/环境变量，形态待定）；
  **无静默回退**（fail-fast 一致——rust 内核不可用 = 显式报错，不悄悄切 py）
- 两内核共享同一 AST 契约 + contracts 层语义红线；语言语义单点真理不变
- 迁移期双内核 ≠ compat shim（工作模式定论）——是"同一语言的两个执行后端"，
  选择经协议显式化；**但 Python 内核是迁移期参考/调试路径（临时），非永久残留**
  （终点 = 全量 Rust 化，见 §2.6）
...
### 2.6 终点裁定：全量 Rust 化（2026-09-10 用户，重新定义 P9 终点）

> **用户裁定（2026-09-10）**：Rust 部分任务完成后，允许开启新的评估和新的自主
> 执行模式，评估**全核心逻辑、全量 Rust 化**。保留关键部分的 Python 接口，以
> 提供灵活性和供 Python 使用的入口能力。当我们证明绝大部分关键核心逻辑都可以
> Rust 化的时候，就可以**全量转向 Rust**，而不必再保留 Python 双通道和对比。

**终点（全量 Rust 化）+ 迁移路径**：

- **范围演进**：P9 从"Rust 化 VM 执行层"（§2.2 四阶段）扩展为**全核心逻辑全量
  Rust 化**（编译/语义/执行/调度/并发等核心面）——Rust 部分（①-④）完成后开启
  **新评估 + 新自主执行模式**，评估全核心逻辑的可 Rust 化面。
- **保留关键部分 Python 接口**（终点态，非废弃）：供**灵活性**（热迭代/调试）+
  **供 Python 使用的入口能力**（Python 侧调用 Rust 核心的入口）。即终点态 =
  Rust 核心 + 关键部分 Python 接口层（入口/灵活性），**非** 100% 无 Python。
- **废弃 Python 双通道/对比（触发条件）**：当**证明绝大部分关键核心逻辑可 Rust
  化**（差分等价零差异 + 全量 pytest 零回归 + 覆盖面实证）→ **全量转向 Rust**，
  废弃 Python 双内核 + 差分 harness 对比（迁移期安全网退场）。
- **迁移期（Rust 部分）安全网**：双内核协议（§2.3）+ 差分等价 harness（§2.2 ①）
  = 迁移期防护；每步 Rust 落地须差分等价零差异 + 全量 pytest 零回归方可放行。
- **裁定纪律**：全量 Rust 化是**渐进证明**（非一次性）——逐面证明可 Rust 化
  （差分等价）→ 覆盖面达"绝大部分关键核心逻辑" → 触发全量转向 + 废弃双通道。
  每步自主决断 + 详尽记录（WORKLOG）；不可自主决断的语义取舍才上报。
