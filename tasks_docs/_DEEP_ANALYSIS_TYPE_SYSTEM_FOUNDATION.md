...
1. **用户怀疑证实**：7 项表面边界中 6 项（除 #6 语言级限制）同源于"类型身份无单一
   ...
   序列化/水化全生命周期。
2. **文档化记录证实（2026-08-13 补充核验）**：原始类型系统架构文档（2138870a）
   明确设计了相反的路线（结构化递归 TypeRef + 纯函数 substitute + 泛型实参随值走），
   并明言反对"每特化造 spec 注册"路线；M1-M5 迁移只做结构折叠，泛型特化机制未按
   原始意图改造——**这是有据可查的历史妥协**。当前文档声称"类型身份单点真理"
   （02_metadata_ast §2.4）但实现未达，KNOWN_LIMITS/设计边界登记是在未识别统一
   根因前的保守处置。
3. **根治方向（地基层）**：把 `GenericTypeDeclaration.build` 与 `SpecFactory.create_*`
...
6. 本分析**未改动任何代码**；改动方案待后续独立分支实验（分支政策：无法确认边界
