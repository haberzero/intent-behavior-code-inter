...
- **🔴 当前任务（2026-08-16，进行中）**：**文档体系系统化重构与正规化**。
  ① docs/ 全量审计与重构（跟随阶段 A-D 代码事实；章节/讲解顺序/模块化/层次归属重整，
  按 `docs/WRITING_GUIDE.md` + doc-governance Phase 0-8）；② tasks_docs 清理与正规化
  （GOVERNANCE 治理章程已建、PENDING_TASKS/NEXT_STEPS/WORKLOG 已重写、临时文档待删）；
  ③ KNOWN_LIMITS 真实性查验与体系化；④ 根目录清洁（`auto`/`example_api.json` 待删）。
- **✅ 已完成（2026-08-16）**：**内核协议化收尾与双轨收敛（阶段 A-D）**——OOP 侧协议化 /
  双轨收敛 / 判定链双协议化 / KNOWN_LIMITS 26 节终判 / 独立候选（copy·deepcopy 内建落地）。
  全量 3006 passed / 1 skipped 零回归；每阶段独立复核放行 + 真实 LLM 复跑与基线一致。
  详情见 `NEXT_STEPS.md` 最近完成节。
- **⏳ 待用户确认**：阶段 E（宣发前置）范围与节奏（准备性评估已登记
  `PENDING_TASKS.md` VISION-2）。

### 2.2 交接检查单（当前有效）

- [ ] 读 `NEXT_STEPS.md`（当前最紧要 + 最近完成）
