...
| 破坏性重构授权 / 分支政策 / 禁 push | 见 AGENTS.md（权威源）。**分支合并细则**：零风险/边界清晰改进（全量 pytest 零回归 + 复核放行）可**直接合并** unsafe-vibe-dev；大风险/无法确认边界仍走独立分支 + 手动 cherry-pick；判定以"是否确认零风险"为准 |
...
普适性/合理架构设计且经分析确实优于现有体系时，哪怕设计已被文档记录也允许破坏性重构，
 ...
 "零风险直接合并"细则）：无法确认边界/危害程度的破坏性重构 100% 授权在其
