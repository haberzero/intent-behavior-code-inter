...
- **⏳ 下一步（新主线）**：**五大地基完整改造**（llm 可调用类 + LLM 体系彻底协议化（总统一性）
  ...
  **下一 session 开工 = P4b-2b LLMCallable 消费面收敛**（`run_batch`/`stream` 统一消费 LLMCallable
  实例 + 行为经统一装配入口路由 + 装配上下文按需引入，按 `_code_p4b_assembly.md` §四），详见
...
- [x] 当前基线实跑：`~/miniconda3/envs/ibci/bin/python -m pytest tests/`（以实跑为准，本 session：3028 passed / 1 skipped）
...
- [x] **✅ P4b-2a LLMCallable 统一装配路径实现落地**：_LLMCallableMixin（协议门 → __llm_call__ 装配 dict→LLMCallRequest → 统一 worker）+ 用户 llm 类端到端判别 + fail-fast；全量 3028 零回归；下一步 = P4b-2b（run_batch/stream 统一消费 + 行为收敛）
