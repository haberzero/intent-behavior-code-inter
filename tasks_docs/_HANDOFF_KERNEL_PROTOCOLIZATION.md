...
- 阶段 E（宣发/推广）的具体范围与节奏（理论清理完成后与用户对齐）。
  **准备性评估（2026-08-16，不实施）**：
  - E1 性能基准：`bench` 编译基准已有（PT-FEAT-5）；运行期/真实场景基准
    （LLM 编排/循环/并发/长程序）缺失——评估后按用户确认范围补；
  - E2 工具链：idbg 内省可用；断点/单步调试器与包管理缺失——宣发最小集待评估；
  - E3 文档旅程：现状已有 guide 00-07（环境/配置/首调用/错误/意图/LLM 函数/
    多步/测试）+ howto 05 篇（调试/试用/生成器/并发/插件）+ examples 03 组
    （入门/模块/进阶）——快速上手与 LLM 编排教程已具雏形，补齐度按宣发范围定；
  - E4 版本：0.2.0（2026-08-11）——理论清理完成后评估 0.3.0。
