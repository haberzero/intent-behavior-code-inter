...
## 4. KNOWN_LIMITS 26 节归类初稿（下一智能体复核，非终判）

| 节 | 主题 | 初稿归类 | 备注 |
|----|------|----------|------|
| §一 | 可调用类实例 `__call__` | 设计定案 | 已 CPS 化（_ClassInstantiateDrive/_UserCallDrive），文档核验 |
| §二 | Enum | 设计定案 | 实例化枚举=独立设计候选（维持） |
| §三 | Uncertain 哨兵 | 设计定案 | |
| §四 | 行为表达式不可 return | 设计定案 | 编译期拦截已实现 |
| §五 | 引用语义局限 | 复核候选 | |
| §六 | 子类 auto-init | **修复候选** | B4 声明化后收敛 |
| §七 | auto/fn/any 对比 | 设计定案 | 已收敛（方向 A） |
| §八 | 容器多类型声明 | 复核候选 | |
| §九 | 废弃语法 | 设计定案 | 硬错误 |
| §十 | 泛型与容器限制 | 复核候选 | 与类型地基后续对齐 |
| §十一 | switch 约束 | 设计定案 | |
| §十二 | intent_context 陷阱 | 设计定案 | |
| §十三 | @ 意图行为 | 设计定案 | |
| §十四 | 用户类能力差距 | **修复候选** | #2 运算符双路径随阶段 A 消解；#1 边界已收敛 |
| §十五 | DDG 并发边界 | 复核候选 | 与调度器统一相关 |
| §十六 | MOCK 限制 | 设计定案 | |
| §十七 | 设计排除语法 | 设计定案 | |
| §十八 | 循环导入 | 设计定案 | Python 同语义 |
| §十九 | 插件隔离 | 设计定案 | |
| §二十 | llmexcept retry 文件写禁 | 设计定案 | 安全取舍 |
| §二十一 | 布尔上下文行为表达式 | 复核候选 | |
| §二十二 | 通信原语面 | 复核候选 | |
| §二十三 | 递归深度 | 设计定案 | 环境限制根因保留 |
| §二十四 | 生成器消费同步阻塞 | **修复候选** | generic_next 协作化（设计已明确方向） |
| §二十五 | yield from 序列委托静态乐观 | **修复候选** | 编译期生成器/序列区分（设计已明确方向） |
| §二十六 | 用户协议与 impl 限制 | 设计定案 | 本轮新登记，fail-fast 边界 |
...
- 是否将"实例化枚举""供应商感知思考禁用"等既有独立候选并入后续窗口。
