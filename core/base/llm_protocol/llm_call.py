...
# ---------------------------------------------------------------------------
# MOCK 指令哨兵（LLM 结果通道的失败/重试标记，供应商无关）
# ---------------------------------------------------------------------------

#: MOCK:REPAIR 指令首轮返回的哨兵：消费方（llm_executor / mock_scenario）据
#: 此触发重试语义（repair 尝试）；替代散落字符串字面量，避免"靠凑巧相等"。
MOCK_REPAIR_SENTINEL = "__MOCK_REPAIR__"

#: MOCK 歧义哨兵（FAIL / 歧义指令）：消费方据此次轮触发歧义重试语义。
