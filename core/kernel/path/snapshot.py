...
Per ADR-017：集中化"save_state 的资产外化到哪个目录、如何命名"的布局决策，
历史上内联在 host/service.py（`save_path + ".assets"` 字符串拼接）。
"""
...
class SnapshotLayout:
    ...
    历史上 host/service.save_state 内联 `abs_path + ".assets"` + `os.path.join(asset_dir, f"{uid}.txt")`；
