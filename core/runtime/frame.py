...
def set_in_thread_task(value: bool):
    """设置线程任务上下文标记，返回 Token（用于 reset）。"""
    return _current_in_thread_task.set(value)


def reset_in_thread_task(token) -> None:
    """重置线程任务上下文标记。"""
    _current_in_thread_task.reset(token)
