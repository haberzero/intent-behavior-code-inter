...
Kernel-native IBCI ``world_model`` 模块实现：世界模型 KB（``knowledge`` 值的
KB 面——facts 事实日志 + vocab 治理词表）的**磁盘面**。

- 提供 IBCI 脚本可见的自由函数：``load_kb(path)`` / ``save_kb(kb, path)``。
- 磁盘形态 = **内容寻址 artifact**（单 JSON 文件）：
  ``{schema_version, content_hash, facts[], vocab{...}, seq}``——JSON 是
  传输格式，运行时模型 = 加载后水化的活 KB 值（非文件本体）。
- ``content_hash`` = canonical 载荷（facts/vocab/seq）的 sha256 全摘要：
  **内容即身份**（同内容不同排版 = 同 hash；加载时重算比对 = 完整性验证）。
