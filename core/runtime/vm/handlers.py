...
``node_protection[cond_expr] = llmexcept_uid`` 仍存在，供
``VMExecutor._apply_protection_redirect`` 在 ``vm_handle_IbFor`` 内部对每次
条件表达式求值进行重定向。
"""
...
def vm_handle_IbLLMExceptionalStmt(executor, node_uid: str, node_data: Mapping[str, Any]):
    ...
    try:
        while frame.should_continue_retrying():
            ...
            # C11：执行 target；CPS 路径优先，否则 fallback
            # （不需要 bypass_protection=True：正则情形 target 不在 node_protection；
            #  条件 for 情形由 _apply_protection_redirect 的活跃帧检测防止重定向。）
