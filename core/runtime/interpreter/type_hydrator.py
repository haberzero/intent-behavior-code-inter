...
from core.domain.types.descriptors import (
    MetadataRegistry, TypeDescriptor, PrimitiveDescriptor, 
...
class TypeHydrator:
    ...
    def _create_shell(self, uid: str) -> TypeDescriptor:
        ...
        elif kind == "FunctionType":
            descriptor = FunctionMetadata(name="callable")
        elif kind == "ClassType":
            parent_name = data.get("parent_name")
            descriptor = factory.create_class(name, parent=parent_name)
        elif kind == "BehaviorType":
            descriptor = factory.create_primitive("behavior")
        elif kind == "BoundMethodType":
