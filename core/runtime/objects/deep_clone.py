...
def try_deep_clone(
    ...
    if isinstance(val, IbNone) or (
        isinstance(val, IbValue) and _value_base_name(val) in ("int", "float", "str", "bool", "vector", "run_result")
    ):
        # vector / run_result = 不可变值类型（固定面 + 无修改方法面），克隆语义
        # = 原对象本身（引用复用，与 int/str 同纪律）
        return val
