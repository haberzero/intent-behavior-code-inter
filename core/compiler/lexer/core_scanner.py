...
class CoreTokenScanner:
    ...
    def check_eof_state(self):
        ...
        if self.sub_state == SubState.IN_STRING:
            self.issue_tracker.error("Unexpected EOF while scanning string literal", self.scanner, code="LEX_001")
        if self.sub_state == SubState.IN_BEHAVIOR:
            self.issue_tracker.error("Unexpected EOF while scanning behavior description", self.scanner, code="LEX_001")
    ...
    def _scan_normal_char(self, tokens: List[Token]) -> bool:
        ...
        if char == '\\':
            if self.scanner.peek() == '\n':
                ...
                self.issue_tracker.error(f"Unexpected character '\\' or invalid escape sequence", self.scanner, code="LEX_002")
                return False
        ...
        self.issue_tracker.error(f"Unexpected character '{char}'", self.scanner, code="LEX_002")
        return False
    ...
    def _scan_string_char(self, tokens: List[Token]):
        ...
        if char == '\n':
            self.issue_tracker.error("EOL while scanning string literal", self.scanner, code="LEX_003")
            return
    ...
    def _scan_complex_access(self, tokens: List[Token]):
            ...
            if subscript_depth == 0:
                ...
                if peek == ']':
                    ...
                    if not self.scanner.is_at_end():
                        ...
                        self.issue_tracker.error("Unclosed string literal in behavior subscript", self.scanner, code="LEX_003")
                        break
        ...
        if subscript_depth > 0:
            self.issue_tracker.error(f"Unclosed subscript '[' in behavior expression (depth: {subscript_depth})", self.scanner, code="LEX_004")
    ...
    def _scan_var_ref(self, tokens: List[Token]):
        ...
        if not self.scanner.is_at_end() and (self.scanner.peek().isalnum() or self.scanner.peek() == '_' or '\u4e00' <= self.scanner.peek() <= '\u9fff'):
            ...
            self.issue_tracker.warning(r"Empty variable reference '$'. Did you mean '\$'?", self.scanner, code="LEX_006")
