...
数据校验内核实现（模块级函数形态）。零内核依赖。

契约（成员面 + 签名）单一权威源 = 内核契约源（contracts/schema.ibci，bind 表达）；
本模块 = 实现面。
...
__all__ = ["validate", "assert_schema", "required_fields", "infer", "coerce"]
...
def validate(data: Dict[str, Any], rules: Dict[str, Any]) -> bool:
    """
    ...
    for field, rule in rules.get("properties", {}).items():
        if field not in data:
            continue
        val = data[field]
        if not _check_field(val, rule):
            return False

    return True


def _check_field(val: Any, rule: Dict[str, Any]) -> bool:
    """递归检查单个字段。"""
...
def coerce(data: Dict[str, Any], rules: Dict[str, Any]) -> Dict[str, Any]:
    """
