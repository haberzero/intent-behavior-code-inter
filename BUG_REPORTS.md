# IBCI 短期问题登记 / 临时 Bug 档案

> **本文件定位**：当前周期内由实测（MOCK + idbg + print 探查）发现、尚未归档的 Bug 与短期 issue 的临时登记簿。
> 内容偏向"行动型记录"——复现步骤、根因猜测、修复进展、影响面。一旦修复并稳定，相关条目应迁出本文件：
> - 修复完成后，正式总结条目沉淀到 `docs/COMPLETED.md`；
> - 仍属于语言层面长期限制的，迁入 `docs/KNOWN_LIMITS.md`（语言限制说明，强调"用法约束 + 设计取向"，对应根因记录）。
>
> **与 `docs/KNOWN_LIMITS.md` 的分工**：
> | 文件 | 定位 | 内容风格 |
> |------|------|----------|
> | `BUG_REPORTS.md`（本文件） | 短期问题登记簿；属于"未归档的现场记录" | 实测复现 + 临时规避 + 修复跟进 |
> | `docs/KNOWN_LIMITS.md` | 正式语言限制说明 | 设计意图、根源描述、推荐写法 |
>
> **最后更新**：2026-04-27（重命名自 `KNOWN_LIMITS.md`；明确分工；同步 Bug #1/#2/#3/#4/#5 状态与最近修复）
