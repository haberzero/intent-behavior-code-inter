...
>
> **最后更新**：2026-04-20（**Steps 1-8 全部完成**：IILLMExecutor 接口、BehaviorAxiom 自主执行、IbDeferred/CallableAxiom/DeferredAxiom、IbLLMFunction 自主执行、IbUserFunction ContextVar 自洽、IbIntentContext 意图公理化、LlmCallResultAxiom + IbLLMCallResult；OOP×Protocol PR-A/PR-B 完成；IntentAxiom 落地；§9.2 SEM_052 编译期快照隔离约束；§9.3 `_last_llm_result` per-snapshot 化；Step 8 架构边界文档化；Bug 修复：IbBool(False) 假值判断、duplicate `_stmt_contains_behavior`、list[str]/dict[K,V] 泛型专化；610 个测试全部通过）
>
...
| **IbLLMFunction** | ✅（`FuncSpec(is_llm=True)`） | 部分（FuncSpec） | ✅ | ✅（Step 4a：通过 `registry.get_llm_executor()` 自主执行） |
| **IbUserFunction** | ✅（`FuncSpec`） | 部分（FuncSpec） | ✅ | ✅（Step 5b：通过 `get_current_frame()` ContextVar 自主获取执行帧） |
| **intent_context** | ✅（`IbSpec("intent_context")`） | ✅（`IntentContextAxiom`） | ✅（`IbIntentContext`，含 `fork()/merge()/push()/pop()`） | ✅（Step 6） |
| **llm_call_result** | ✅（`LLM_CALL_RESULT_SPEC`） | ✅（`LlmCallResultAxiom`） | ✅（`IbLLMCallResult`，含 `is_certain/result_value/retry_hint`） | ✅（Step 7） |

### 尚未完整公理化的类型

| 类型 | 问题描述 |
|------|---------|
| **Intent** | ✅ `IntentAxiom`（`is_class()=True`，完整 vtable）已注册；`IbIntent` 含 `get_content()/get_tag()/get_mode()` 三个公共方法。已完成，见 `docs/COMPLETED.md §1.12`。 |

> 所有内置类型均已完整公理化。上表保留 Intent 条目仅作历史记录。
...
Object（根）
  ...
  ├─ intent_context (IntentContextAxiom)          — 意图上下文对象（Step 6，已完成）
  ├─ llm_call_result (LlmCallResultAxiom)         — LLM 调用结果类型（Step 7，已完成）
...
## 二、核心诊断（已解决）：`behavior` 无法自洽的根因与解决方案

> **注**：本节描述的根本问题已在 Steps 1-5 中完整解决。保留此节作为设计决策的历史背景说明。
...
| Step 4a | `IbLLMFunction` 自主执行（`invoke_llm_function`），移除 `llm_executor` 持有 | `core/runtime/objects/kernel.py`、`core/base/interfaces.py` |
| Step 4b | `ibci_ihost`/`ibci_idbg` KernelRegistry 标准化：`IStateReader` 扩展、三组钩子、`PluginCapabilities.kernel_registry` | `core/kernel/registry.py`、`core/base/interfaces.py`、`core/extension/capabilities.py` |
| BehaviorSpec | `BehaviorSpec(value_type_name)` 编译期返回类型推断 | `core/kernel/spec/specs.py`、`core/kernel/spec/registry.py`、`core/compiler/semantic/` |
| OOP PR-A | IbObject 子类单继承清理，Protocol isinstance 替换，幽灵字段删除 | `core/runtime/interfaces.py`、`core/runtime/objects/builtins.py` |
| OOP PR-B | Impl 类 Protocol 声明性继承移除 | `core/runtime/interpreter/runtime_context.py` 等 |
| **Step 5a** | `IExecutionFrame` Protocol 正式定义于 `core/base/interfaces.py`，给已有结构命名 | `core/base/interfaces.py` |
| **Step 5b** | `ContextVar` 帧注册表（`core/runtime/frame.py`），`IbUserFunction.call()` 通过 `get_current_frame()` 自主执行 | `core/runtime/frame.py`、`core/runtime/objects/kernel.py`、`core/runtime/interpreter/interpreter.py` |
| **Step 6a** | `IntentContextAxiom` 注册到公理体系 | `core/kernel/axioms/intent_context.py`、`core/kernel/axioms/primitives.py` |
| **Step 6b** | `IbIntentContext` 运行时对象，含 `fork()/merge()/push()/pop()/remove()` | `core/runtime/objects/intent_context.py` |
| **Step 6c** | `RuntimeContextImpl` 完整迁移：四个意图相关私有字段统一迁入 `_intent_ctx: IbIntentContext` | `core/runtime/interpreter/runtime_context.py` |
| **Step 6d** | `LLMExceptFrame` 快照清洁化：`saved_intent_ctx` 替代裸 `_intent_top` 引用 | `core/runtime/interpreter/llm_except_frame.py` |
| **Step 7** | `LlmCallResultAxiom` + `LLM_CALL_RESULT_SPEC` + `IbLLMCallResult` 全链路公理化接入 | `core/kernel/axioms/primitives.py`、`core/kernel/spec/specs.py`、`core/runtime/interpreter/runtime_context.py` |

完整实现细节见 `docs/COMPLETED.md`。

---

### 下一步：Step 8-pre + Step 9-11（VM 演进，见 `docs/NEXT_STEPS.md`）

| Step | 内容 | 前提条件 | 状态 |
|------|------|---------|------|
| Step 8-pre | llmexcept 快照隔离约束完整落地（§9.2 SEM_052 + §9.3 `_last_llm_result` per-snapshot） | — | ✅ 完成 |
| Step 8 | 架构边界注释文档化 | 随时可做 | ✅ 完成 |
| Step 9 | VM CPS 调度循环（消除 Python 递归栈） | IExecutionFrame 接口完整（✅） | ⏳ |
| Step 10 | Layer 1 LLM 流水线（DDG 编译器 + LLMScheduler） | IbIntentContext.fork() 完整（✅） | ⏳ |
| Step 11 | Layer 2 多 Interpreter 并发 | ContextVar（✅） | ⏳ |

详见 `docs/NEXT_STEPS.md` 和 `docs/PENDING_TASKS_VM.md`。

---

## 五、远期高风险任务记录

### 5.1 `IbFunction.call()` ContextVar 方案 ✅ 已解决（Step 5）

**已实现**：`ContextVar[IExecutionFrame]`（`core/runtime/frame.py`）替代"外部传入 context 参数"方案。
`IbUserFunction.call()` 通过 `get_current_frame()` 自主获取执行帧，不再依赖外部 context 参数。

并发安全性：asyncio Task 创建时自动复制父 Context，协程间 ContextVar 天然隔离，与 Python 生态（FastAPI/Starlette）完全对齐。

---

### 5.2 Intent 公理化 ✅ 已完成（Step 2.1）

**完成状态**：`IntentAxiom`（`is_class()=True`，完整 vtable）已注册到公理体系；`IbIntent` 持有 `get_content()`、`get_tag()`、`get_mode()` 三个公共方法；`INTENT_SPEC` 加入 `specs.py` 并注册到 `create_default_spec_registry()`。

完整实现细节见 `docs/COMPLETED.md §1.12` 和 `docs/PENDING_TASKS.md §2.1`。

---

### 5.3 `llm_tasks.py` 异步任务系统（内部草稿，不对外暴露）

`core/runtime/async/llm_tasks.py` 是未完成的内部草稿：
- **现状**：同步调用伪装成 Future 模式（无真正并发）；`execution_context=None` 传入存在潜在 NPE
- **状态**：`core/runtime/async/__init__.py` 不导出任何符号，此文件对外不可见
- **触发时机**：Layer 1 LLM 流水线设计完成后，依赖 VM CPS 调度循环（Step 9）才能实现真正异步
- 见 `docs/PENDING_TASKS_VM.md` §⚠️危险悬案

---

## 六、一句话总结

> **公理化/万物皆对象改造全部完成（Steps 1-8）**：IILLMExecutor 接口、BehaviorAxiom 自主执行、ibci_ai 拆分、CallableAxiom/DeferredAxiom/IbDeferred、IbLLMFunction 自主执行、ibci_ihost/idbg KernelRegistry 标准化（Steps 1-4b）；IExecutionFrame Protocol + ContextVar 帧注册表、IbUserFunction 自洽执行（Step 5）；IntentContextAxiom + IbIntentContext + RuntimeContextImpl 完整迁移 + LLMExceptFrame 安全快照（Step 6）；LlmCallResultAxiom + IbLLMCallResult 全链路接入（Step 7）；IntentAxiom 完整公理化（Step 2.1）；llmexcept 快照隔离完整落地：§9.2 SEM_052 编译期约束 + §9.3 `_last_llm_result` per-snapshot 迁移（Step 8-pre）；Step 8 架构边界注释文档化。BehaviorSpec 编译期返回类型推断完成。OOP×Protocol 边界完整清理（PR-A + PR-B）。**610 个测试全部通过。****下一里程碑是 Step 9（VM CPS 调度循环）→ Step 10（LLM 流水线）→ Step 11（多解释器并发）**，见 `docs/NEXT_STEPS.md`。
