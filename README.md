...
IBC-Inter 采用高度解耦的**数据驱动型编译器架构**，核心逻辑分为三层：

### 1. 核心领域模型层 (`core/domain/`)
-   **AST ([ast.py](file:///c:/myself/proj/intent-behavior-code-inter/core/domain/ast.py))**: 100% 纯净且只读的抽象语法树定义。
-   **Symbols ([symbols.py](file:///c:/myself/proj/intent-behavior-code-inter/core/domain/symbols.py))**: 语义符号与类型系统（UTS）的物理实体。
-   **Artifact ([artifact.py](file:///c:/myself/proj/intent-behavior-code-inter/core/domain/artifact.py))**: 编译器与解释器之间的通信契约。

### 2. 生产逻辑层 (`core/compiler/`)
-   **Lexer & Parser**: 负责将源码转换为初始 AST。
-   **Semantic Analyzer ([passes/](file:///c:/myself/proj/intent-behavior-code-inter/core/compiler/semantic/passes/))**: 执行多轮语义分析，填充符号绑定和类型推导侧表。
-   **Flat Serializer ([serialization/](file:///c:/myself/proj/intent-behavior-code-inter/core/compiler/serialization/))**: 核心黑科技。将具有复杂引用关系的内存对象图转换为**扁平化池结构 (Flat Pooling)**，消除内存地址依赖。
-   **Scheduler ([scheduler.py](file:///c:/myself/proj/intent-behavior-code-inter/core/compiler/scheduler.py))**: 负责多文件编译调度与依赖图管理。

### 3. 执行执行层 (`core/runtime/`)
-   **Interpreter ([interpreter.py](file:///c:/myself/proj/intent-behavior-code-inter/core/runtime/interpreter/interpreter.py))**: 核心执行引擎。它**不再依赖**编译器的内部逻辑，而是直接在扁平化的数据池上“查表执行”。
-   **LLM Executor**: 处理提示词构建、参数插值和结果校验。

---

更多详情请参阅：
- [编译器与解释器对齐规范](docs/compiler_alignment_spec.md) (核心契约)
- [语言规范](docs/ibc_inter_language_spec.md) (语法与 Class 系统)
- [内核调试器指南](docs/core_debugger_guide.md) (观测与追踪)
- [单元测试指南](docs/unit_testing_guide.md) (开发与验证)
