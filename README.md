...
与 `__to_prompt__` 协议相关的，还有 `__from_prompt__` 协议，用于将模型返回的字符串转换为类对象；以及 `__outputhint_prompt__` 协议，用于注入并约束模型返回的字符串的格式。
...
llmexcept 机制与 IBC-Inter 的 `__to_prompt__` 以及 `__from_prompt__` 协议紧密相关，详情请见 [IBCI_SPEC.md](docs/IBCI_SPEC.md)
...
- [IBC-Inter语法说明手册](docs/IBCI_SPEC.md) (语法与类型系统)
