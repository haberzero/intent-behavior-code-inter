"""
tests_v2/e2e/test_higher_order.py
==================================
...
``tests_v2/contracts/test_scope_semantics.py`` 中（INV-CELL-* / INV-SNAPSHOT-*）。
本文件仅保留 e2e 层面的独有场景（如容器就地突变、重入性、factory 模式等）。
"""
from tests_v2.conftest import run_ibci, compile_ibci, expect_compile_error, AI_MOCK_PREFIX
