...
class Parser:
    ...
    def call(self, left: ast.Expr) -> ast.Expr:
        start_token = self.previous() # LPAREN or IDENTIFIER? No, previous is LPAREN
        
        # Capture intent at the START of call parsing to ensure it attaches to the outer-most call
        # in a nested structure like `outer(inner())`.
        saved_intent = None
        if self.pending_intent:
            saved_intent = self.pending_intent
            self.pending_intent = None
        ...
        return self._loc(ast.Call(func=left, args=arguments, keywords=[], intent=saved_intent), start_token)
    ...
    def behavior_expression(self) -> ast.BehaviorExpr:
        ...
        while not self.check(TokenType.BEHAVIOR_MARKER) and not self.is_at_end():
            if self.match(TokenType.RAW_TEXT):
                content_parts.append(self.previous().value)
            elif self.match(TokenType.VAR_REF):
                var_name = self.previous().value # e.g. "$x"
                variables.append(var_name)
                content_parts.append(var_name) # Keep it in content or handle separately?
            else:
                # Should not happen in behavior mode if lexer is correct
                raise self.error(self.peek(), "Unexpected token in behavior expression.")
