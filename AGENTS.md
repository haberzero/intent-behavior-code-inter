...
> **大范围破坏性重构的分支政策（用户 2026-08-03 明确，硬原则；2026-08-11 补充"零风险直接合并"细则）**：对于**无法确认边界、无法判断危害程度**的破坏性重构，**100% 授权在其独立分支上**（不污染 `main` 与 `unsafe-vibe-dev`，不污染任何计算机环境与用户目录），允许在该独立分支进行**任意程度的破坏性实验与重构**。**只有当即使该独立隔离分支也无法经自主实验确定工作路线时**，才将相关任务配置为阻塞。**独立分支禁止直接合并到 `unsafe-vibe-dev` 或 `main`**；只有在确认完整技术路线后，才允许单独进行手动 `unsafe-vibe-dev` 代码更新。**永远不允许触碰主干分支（`main`）的代码。**
> **分支合并细则（用户 2026-08-11 明确）**：**经充分验证零风险/边界清晰的改进**（全量 pytest 零回归 + 复核放行，无对外契约/架构级风险），允许**直接合并**到 `unsafe-vibe-dev`；**识别到大风险/无法确认边界**的破坏性重构，仍按上述"独立分支 + 手动 cherry-pick 单独更新 unsafe-vibe-dev"执行。判定以"是否确认零风险"为准，非以改动规模。
...
>
> **设计阶段文档放置（用户 2026-08-03 明确）**：任何**设计/决策/用户裁定**（如架构、接口、AST 变更、并发正确性等）在**设计阶段先写任务控制文档 `tasks_docs/`**（如 `tasks_docs/THREAD_DESIGN_REVISION.md`），**不写进技术手册 `docs/`**；待实现落地后，按 `docs/` 治理纪律择机收敛写入技术手册。此规则是"重大架构决策写入 `docs/architecture/`"（见 §不要做的事）的**设计阶段前置**，二者不冲突（先 tasks_docs/ 设计，落地后 docs/ 记录）。
