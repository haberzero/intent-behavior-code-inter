...
> - **无目的审视**（`aimless-review`）为**最低密度**的背景过程，产出潜在参考入 `tasks_docs/AIMLESS_REVIEW.md`，并周期性事实回顾防过期。
...
>
> **设计阶段文档放置（用户 2026-08-03 明确）**：任何**设计/决策/用户裁定**（如架构、接口、AST 变更、并发正确性等）在**设计阶段先写任务控制文档 `tasks_docs/`**（如 `tasks_docs/THREADING_DESIGN.md`），**不写进技术手册 `docs/`**；待实现落地后，按 `docs/` 治理纪律择机收敛写入技术手册。此规则是"重大架构决策写入 `docs/architecture/`"（见 §不要做的事）的**设计阶段前置**，二者不冲突（先 tasks_docs/ 设计，落地后 docs/ 记录）。
