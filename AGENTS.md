...
> `docs/` 只包含**给人类阅读**的架构设计、语法说明、使用指导。智能体可参考并实时更新，但**不得在其中写入纯智能体信息**（工作流 / 过程 / agent 指令 / skill 引用）。智能体信息只存在于 `AGENTS.md` 与 `.opencode/skills/` 工作流层。治理 docs/ 时按其自检（见 `doc-governance`）。
