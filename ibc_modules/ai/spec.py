from core.foundation.module_spec_builder import SpecBuilder

spec = (SpecBuilder("ai")
    .func("set_config", params=["str", "str", "str"])
    .func("set_retry_hint", params=["str"])
    .func("set_retry", params=["int"])
    .func("set_timeout", params=["float"])
    .func("set_general_prompt", params=["str"])
    .func("set_branch_prompt", params=["str"])
    .func("set_loop_prompt", params=["str"])
    .func("set_return_type_prompt", params=["str", "str"])
    .func("get_return_type_prompt", params=["str"], returns="str")
    .func("set_decision_map", params=["dict"])
    .func("get_decision_map", returns="dict")
    .func("get_last_call_info", returns="dict")
    .func("get_scene_prompt", params=["str"], returns="str")
    .func("set_scene_config", params=["str", "dict"])
    # Global Intent Management
    .func("set_global_intent", params=["str"])
    .func("clear_global_intents")
    .func("remove_global_intent", params=["str"])
    .func("get_global_intents", returns="list")
    .func("get_current_intent_stack", returns="list")
    .func("mask", params=["str"])
    .build())
