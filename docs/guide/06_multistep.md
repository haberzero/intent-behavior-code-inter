...
> 本章是 IBCI 多步骤 LLM 工作流的入门教程。面向已掌握 LLM 可调用类定义和意图系统的 IBCI 用户。覆盖如何串联多个 LLM 调用、用 `for @~` 实现 AI 驱动循环、以及结合 `fs` 模块和 `llmexcept` 构建健壮的处理管道。
...
前提：你已完成 [05 · 定义和使用 LLM 可调用类][] 的学习，能封装和调用 LLM 可调用类。
...
每一步都可能出现 LLM 解析失败，需要容错处理。本章构建这个内容处理管道，并为每一步加上容错保护。
