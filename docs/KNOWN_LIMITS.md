...
## 二十七、LLM 函数 `-> void` 返回边界

> **类型**：已知边界（当前实现的限制，可能随演进改变）。

**LLM 函数返回类型声明为 `void` 时，编译通过但运行期解析抛 `LLMParseError`**——`void` 无输出解析目标，语言当前**未声明支持**（既未正式支持也未编译期拦截；是否转为"丢弃输出"或"编译期清晰错误"待评估）。

**规避建议**：LLM 函数应声明可解析的返回类型（标量/容器/用户类——解析规则见 `docs/syntax/08_llm_functions.md` §8.4）；需要纯副作用调用时使用行为表达式（`@~ ... ~`）而非 `llm ... -> void`。
