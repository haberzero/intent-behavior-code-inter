...
**编译期绑定检查**：构造器调用的参数绑定错误（缺必填/位置超限/未知具名/重复具名）在编译期以 `SEM_MISSING_REQUIRED_ARG` / `SEM_TOO_MANY_POSITIONAL` / `SEM_UNKNOWN_KEYWORD` / `SEM_DUPLICATE_KEYWORD` 拦截（携带 ibci 源行列）。auto 构造器签名 = 继承链有效无默认值字段，编译期与运行期共享同一覆盖/位置规则（单一源，不双写）。边界形态：① 链上无必填字段（全部字段带默认值，含子类同名把父类无默认字段覆盖为带默认值）= **零参构造器**；② 链上无任何字段 = 继承祖先显式 `__init__`（祖先无显式构造器时为零参）——两形态编译期/运行期同判据。
...
### 10.1 `dict` 键类型在下标访问时静态校验（动态键运行期不校验）

`dict[str, int]` 的下标键类型在**编译期**做可赋值校验：静态错位（`d[42]` on `dict[str,int]`）报
`SEM_TYPE_MISMATCH`；键或目标任一侧动态（`any`/`auto`）放行（运行期裁决——运行期下标访问仍不校验键类型，
错误键按 Python 语义抛 `KeyError`，可捕获）。
...
`*expr` 展开实参：特化容器（`list[int]`）展开时元素类型与目标形参做可赋值校验（`list[str] *-> f(int)` 编译期拦截）；每个 `*expr` 的目标形参偏移 = 其**之前**显式位置实参数量（中置/前导星 `f(10, *l, 30)` 亦正确，与运行期顺序展开一致）。裸容器/动态/数量不足由运行期裁决（静态数量未知是本质限制）。
...
## 二十六、ihost 子运行（run_file / run_code / run_isolated）值通道与性能边界

> **类型**：已知边界（当前实现的限制，可能随演进改变）。

**子运行 = 独立子进程（进程级隔离）+ JSON 结果协议。** `ihost.run_file` /
`ihost.run_code` / `run_isolated` 启动新的 IBCI 子进程（全新 Engine 实例，独立
地址空间）执行候选代码。隔离面：变量不跨边界继承（父→子仅经显式 file 传递）/
LLM 配置继承（spawn 时点快照）/ fs 沙箱（相对路径锚定子 project_root，越出 =
`RUN_PERMISSION_ERROR` 拒绝）/ 防卡死（`collect_timeout`）/ 资源限制（可选，POSIX
`rlimit`）。

**值交换 = JSON 原生值（序列化边界）。** 子进程经 JSON 结果协议导出全局变量：
仅 **JSON 原生值**（`str`/`int`/`float`/`bool`/`list`/`dict`/`None`）可导出——
用户类实例 / 可调用值（`fn_callable`/`behavior`）/ 复杂结构不导出（静默排除，
`run_isolated` 面 = 取不到该变量；`meta.eval` 面 = 显式 fail-fast 上抛）。

**性能**：每次子运行调用构造一个全新子进程 + 全新子引擎——**候选验证场景
（非热循环）充分**；热循环高频调用子运行的性能上修挂数据平面性能线（缓存
预编译 / 真 JIT），非当前范围。

**应对**：以候选代码执行 + 机械判定为目的（三门管线：编译门 / 隔离门 / 判定门，
操作指南见 `docs/howto/run_code_safely.md`）时当前边界充分；需取回复杂值
（用户类实例等）时不经子运行值通道（改用显式 file 交换或父进程内执行）。

---

## 二十七、行为值（`@~...~`）错误的惰性显形面（future 按需解析）

> **类型**：已知边界（当前实现的限制，可能随演进改变）。

行为表达式的 LLM 调用经 worker 线程异步派发，**值 future 按需解析**——错误
（provider 失败 / 确定性模式拦截 `RUN_DETERMINISTIC_LLM_CALL` 等）在**值被
实际使用时**才显形：

- **值被使用**（`print(x)` / 参与运算 / 赋给后续表达式）或**顶层无 try 包裹**
  → 错误正常 fail-fast 上抛（顶层 = run 失败；try 块内 = `except` 捕获）；
- **值未被引用**（如 `str x = @~...~` 后 `x` 从未使用）→ 错误**静默吞没**
  （run "成功"，值取不到）——既有语义，与 LLM 调用成败无关（mock 失败/真
  失败/确定性拦截同型）。

**应对**：需要错误显形的 LLM 值**必须被消费**（查询/打印/参与后续计算）；
确定性执行模式（`--deterministic`）的零 LLM 不变量**结构性成立**（拦截在
provider 调用前——被拦调用不发出，llm_calls=0 凭证不受本边界影响），本边界
仅影响"被拦调用的错误是否打断 run"的显形面。

---

## 二十八、Int 值模型有界契约（i64）

> **类型**：已知边界（当前实现的限制——Rust 内核值模型；设计评估后维持，见成因）

**Int = i64 有界（±9.2e18）**；算术经 checked 运算——超出 i64 范围（如
`2 ** 100`、i64 加法溢出）= 显式 `OverflowError`（fail-fast，无静默回绕）。

```ibci
# ✅ 正常范围
print(2 ** 62)          # 4611686018427387904
# ❌ 溢出 = 显式错误（无静默）
print(2 ** 100)         # OverflowError
```

**成因**（i128 值模型评估裁决）：① i64 覆盖一切现实量级（计数/索引/尺寸），
大整数非 DSL 典型场景；② 溢出 = 显式错误优于静默错误值（契约正确性）；③
i128/任意精度 = JSON/serde/typed 通道/差分对齐的全局复杂度，远超边际收益；
④ 可扩展路径保留：若未来确需大整数（哈希/加密等），作为独立值类型（BigInt）
引入，不改 Int 的 i64 契约——避免全局值模型负担。

**应对**：需大整数场景 = 显式设计 BigInt 值类型（未来）；当前溢出 = 显式错误
（fail-fast 契约，已登记差分面 int_overflow——Python 参考任意精度 vs Rust
有界，零语料偏离）。
