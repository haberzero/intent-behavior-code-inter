...
## 二十六、ihost 子运行（run_file / run_code）隔离与性能边界

> **类型**：已知边界（当前实现的限制，可能随演进改变）。

**子运行隔离 = 进程内子环境，非对抗性代码安全边界。** `ihost.run_file` / `ihost.run_code`
在**同一进程**内启动隔离子引擎执行候选代码——隔离面为：变量不跨边界继承（父→子
仅经显式 file 传递）/ LLM 配置继承（spawn 时点快照）/ fs 沙箱（相对路径锚定子
project_root，越出 = `RUN_PERMISSION_ERROR` 拒绝）/ 防卡死（`collect_timeout`）。
**无进程级隔离**——子代码与父共享同一进程地址空间。

**成因**：设计取向（选项 A = 调用方表达治理 + 语言原语）下，治理策略由调用方 IBCI
代码表达（调用方决定运行什么代码），内核只提供隔离基元 + 安全保证。威胁模型 =
**受信任候选代码**（自生成/结晶候选，非对抗性）；进程级对抗性隔离属内核工程化
（VISION-6 档 B 上修输入），非当前范围。

**性能**：每次 `run_file` / `run_code` 调用构造一次全新子引擎（scheduler + registry
+ 合成入口）——**候选验证场景（非热循环）充分**；热循环高频调用子运行的性能上修
