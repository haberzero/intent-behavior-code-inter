...
> **⚠️ 已归档（2026-06-25 文档体系整理）**
> 本文档是类型系统重构的**设计原文**（架构推演、语言学动机、第一性原理推导）。
> 其作为"现行类型系统参考"的角色**已被 `docs/design/TYPE_SYSTEM_DESIGN.md`（与代码对齐的正式设计）完全取代**。
> 保留于 `docs/design/` 仅供追溯设计动机。请勿据此判断当前实现状态。
> 当前代码已完成 **M1–M5** 全部里程碑（2026-05-08）；完成时间线见 `tasks_docs/COMPLETED.md`。
