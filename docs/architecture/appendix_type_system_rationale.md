...
> **已归档**：本文档是类型系统重构的**设计原文**（架构推演、语言学动机、第一性原理推导）。
> 现行类型系统参考见 `docs/architecture/03_type_system.md`（与代码对齐的正式设计）。
> 本文仅供追溯设计动机，请勿据此判断当前实现状态。
...
这一段定义应该写进 `docs/architecture/03_type_system.md` 的第一行，作为**所有未来设计决策的判断标准**——任何新提案都要回答："这违反了静态/名义/强/渐进/空安全/LLM 扩展中的哪一条？为什么例外？"
...
我假设你不会一次性全换（即使你说不在意成本，工程上也应当分阶段以保证全量测试始终绿色）。我建议的最小可独立推进的阶段：

| 阶段 | 内容 | 风险 | 测试影响 |
|------|------|------|---------|
| **M1** | 引入 TypeRef 类，所有 spec 字段从 `xxx_name+xxx_module` 改为 `xxx_ref: TypeRef`。**保留** IbSpec 子类层级 | 中 | 全程保持全量通过 |
