...
> 本文是 IBCI 语言设计的**体系化演进评估与规划**文档：从编程语言设计、类型理论、
> 工程抽象与内核一致性的角度评估 IBCI 的类型系统与协议机制，并给出演进路线。
> 面向设计研究者；已落地演进（协议注册表/判定协议化等）以
> `docs/architecture/03_type_system.md` §4 为权威，本文不替代架构文档。
