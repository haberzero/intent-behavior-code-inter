...
> ⚠️ **Known Limit (KNOWN_LIMITS.md §三)**：`fn` 变量与可调用类实例存在已知设计问题，不建议使用。
...
> ⚠️ **Known Limit (KNOWN_LIMITS.md §二)**：`try`/`except` 存在已知设计问题，**强烈不建议使用**。处理 LLM 不确定性请用 `llmexcept`（见第 10 节）。
...
> ⚠️ **Known Limit (KNOWN_LIMITS.md §七)**：`return @~ ... ~` 是**禁止写法**，会产生 `SEM_003` 编译错误。
...
> ⚠️ **Known Limit (KNOWN_LIMITS.md §三)**：`fn` 变量存在已知设计问题，不建议使用。
...
> ⚠️ **Known Limit (KNOWN_LIMITS.md §四)**：
> - 枚举成员目前仅支持 `str` 类型。
> - 不支持枚举迭代和数量查询。
