...
- 运行时旧值类（`IbInteger` / `IbList` / `IbBehavior` 等）目前作为 `IbValue` 子类的兼容包装层保留，正在收敛（**优先级：中**）：已将 `isinstance(obj, IbXxx)` 分派替换为 `obj.ib_class.name` 检查，工厂新增 `create_tuple` / `create_dict`；类名本身保留供内部行为实现，不再作为对外接口。
