...
>
> 命名 LLM 调用由 **LLMCallable 可调用类实例** 承载——定义一次、任意调用，经统一装配入口消费。
...
> 装配字典中**未列出的未知键被静默忽略**（不报错、不参与装配）。仅 `user_prompt` 为必需
> （缺失即 fail-fast）；拼写错误的安全键（如 `expected_type` 写成 `expected_typo`）会被当作
> 未知键忽略，导致按默认字符串解析——声明类型时请核对键名。
...
- **`__retry__`**（可选）：`func __retry__(self) -> dict` —— 声明**调用级默认重试策略**（高阶化 retry）。返回策略字典：
  ...
  - `hint`（`str`）：每轮失败后注入下一轮调用的补充要求（经标准多轮对话的 `message_history` 回喂，不拼入系统提示词）。
