...
retry body 内**禁止文件写/删操作**（`fs.write` 与 `remove`），编译期以 `SEM_LLMEXCEPT_FILE_WRITE` 拦截直接与间接调用，运行时兜底动态分派等漏检情形。只读操作（`open` / `read` / `read_bytes` / `exists`）允许。
...
llmexcept:
    fs.write(fh, "mutated", overwrite_flag="overwrite")   # 编译错误 SEM_LLMEXCEPT_FILE_WRITE：retry body 中禁用文件写/删
    retry "请只返回摘要"
```
...
### 10.5 llm 可调用类的调用级重试策略（`__retry__`）

LLM 可调用类可通过可选协议方法 `__retry__` 声明**调用级默认重试策略**（面向对象形态，见 `docs/syntax/08_llm_callable.md` §8.4）：
...
调用失败（LLM 结果不确定）时按策略自动重试：失败响应与解析错误随 `hint` 经标准多轮对话的 `message_history` 回喂，不拼入系统提示词；达到 `max_retry` 仍不确定，结果交语句层（`llmexcept` 接管 / 无保护时抛解析错误）。

**与 `llmexcept` 的分工**：

| 维度 | `llmexcept`（语句级） | `__retry__`（调用级） |
|------|----------------------|----------------------|
| 作用对象 | 被保护的语句/表达式执行窗口 | LLM 可调用类实例的本次调用 |
| 重试语义 | 存快照 → 恢复 → 重新求值被保护语句（窗口状态一致） | 同一调用请求按策略重放（失败轮回喂多轮对话） |
| 纠错注入 | `retry "..."` 指令（动态） | `hint` 声明（静态策略） |

两者互补：`__retry__` 声明了类自身的默认重试；策略轮耗尽后仍不确定时，若调用点处于 `llmexcept` 保护窗口内，由帧机制接管继续重试。行为语句（`@~ ... ~`）不实现 `__retry__`——其默认重试由帧机制提供。
---
