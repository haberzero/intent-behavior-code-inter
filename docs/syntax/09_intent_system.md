...
- **意图效果取决于模型服从性**：意图以"必须严格遵守的意图约束"清单注入
  system prompt，是**指令性约束**而非程序化强制。对低指令遵循度的模型，
  风格类意图（如"用冷酷无感情的口吻"）可能不被严格服从；`int/bool` 等
  输出**类型约束**（格式服从）则稳定生效。这是模型特性，非 IBCI 缺陷。
  若需更强约束，可把意图直接写入 `@~` 表达式的提示词正文。
