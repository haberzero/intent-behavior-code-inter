...
### 1.2 BaseCompilerTest 基类

针对不涉及解释器运行的**纯编译器测试**，建议继承自 `tests/compiler/base.py` 中的 `BaseCompilerTest`。
- **快速编译**：提供 `assert_compile_success()` 和 `assert_compile_fail()` 方法。
- **结果内省**：自动处理 `CompilationArtifact` 到 `CompilationResult` 的提取，方便检查侧表和符号池。
