...
**核心规则：**
1. **推荐用法**：意图注释**强烈建议**仅附加在命名的 LLM 函数调用上。
2. **非 LLM 调用**：如果附加在非 LLM 调用（如普通函数或赋值语句）上，解释器会报出 **Warning** 警告，表示该意图未被任何 LLM 场景消耗，但不会终止程序。
3. **不支持行为描述行**：意图注释不支持附加在行为描述行（@~）上。
...
2. **内容书写**：行为描述内容被视为纯文本（Raw Text）。如果需要在行为描述行中书写 `~` 或 `$`，请使用反斜杠转义：
    ...
    - `\~`：表示波浪号 `~`（作为普通文本，而非结束符）。
    - 其他字符前的反斜杠 `\` 将保持原样，不做转义处理。这意味着可以直接书写包含 `\` 的路径或文本。
