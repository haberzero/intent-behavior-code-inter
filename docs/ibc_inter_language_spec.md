...
LLM函数是一种特殊的函数，用于调用LLM模型执行自然语言任务。LLM函数支持多种返回值类型（int, float, bool, str, list, dict），默认为 str。参数仅支持 int 与 str 类型，不支持容器类型。
...
意图注释在 LLM 调用时动态注入系统提示词，作用于运行时而非定义时。

**核心规则：**
1. **支持对象**：
    - **命名 LLM 函数调用**：`@ 意图内容` 直接作用于随后的 LLM 函数。
    - **行为描述行 (@~)**：当行为描述行处于赋值语句（如 `str res = @~...~`）或独立表达式语句中时，前置的意图注释会自动注入到该行为行的执行上下文中。
2. **Lambda 化行为**：如果行为描述行被赋值给 `callable` 变量，意图注释会在该变量被**定义时**捕获，并在后续**调用时**生效。
3. **非 LLM 场景警告**：如果附加在无法消耗意图的语句（如纯算术运算）上，解释器会报出 **Warning** 警告。

```ibc-inter
@ 翻译时请使用古风词汇
callable 翻译器 = @~ 将 $input 翻译成中文 ~
...
4. **内容书写**：行为描述内容被视为纯文本（Raw Text）。支持使用 `\$` 和 `\~` 进行转义。
