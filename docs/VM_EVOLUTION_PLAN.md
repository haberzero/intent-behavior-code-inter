...
> **基准状态**（2026-04-29）：**996 个测试通过**；Step 1–8 全部完成；**M1 + M2 + M3a + M3b + M3c + M3d + M4 + M5a + M5b + M5c + M6 已完成**；**编译器深度清洁 Phase 1–4（C6/C7/C8/C9/C10/C12/C13/C14）全部完成**（CPS dispatch table 覆盖 43 个 AST 节点类型；所有显式 `fallback_visit()` 调用归零；`_vm_assign_to_target()` CPS generator helper 替代 assign_to_target 递归穿透；`IbLambdaExpr.free_vars` 编译期填充；`cell_captured_symbols` 侧表；Signal→CSE 桥已收缩为单层边界兼容包装）。  
> **剩余技术债**：**C5**（`ControlSignalException` 类本体删除，需调整测试契约）+ **C11**（`node_protection` 侧表重构，编译器+序列化+运行时三层，独立 PR）。  
...
> **关联文档**：`docs/PENDING_TASKS_VM.md`（详细设计）、`docs/NEXT_STEPS.md`（近期任务）、`docs/COMPLETED.md`（已完成记录）、`docs/VM_SPEC.md`（正式 VM 规范）、`docs/DEFERRED_CLEANUP.md`（延迟清理任务）。
...
```
python3 -m pytest tests/ -q --tb=short   # 996 passed（2026-04-29，编译器深度清洁 Phase 1–4 完成后）
```
...
```
已完成（Step 1–8 + M1 + M2 + M3a–M3d + M4 + M5a–M5c + M6 + Phase 1–4 编译器深度清洁 C6–C14）
  └─── 当前状态（基准：996 tests）
         │
         ...
         └─── ✅ M5c：dispatch-before-use                             [已完成 2026-04-29]
                ...
                └─── ✅ M6：可移植性参考实现 + 合规测试套件（Step 12） [已完成 2026-04-29]
                       │
                       └─── ✅ 编译器深度清洁 Phase 1–4（C6–C14）     [已完成 2026-04-29]
                              │
                              ├─── ⏳ C5：ControlSignalException 类本体删除（测试契约调整）
                              ├─── ⏳ C11：node_protection 侧表重构（独立 PR）
                              └─── ⏳ 下一功能里程碑（见下方详细规范）
```

**当前优先路径**：编译器债务清洁 Phase 1–4 全部完成（2026-04-29）。CPS dispatch table 覆盖 **43 个 AST 节点类型**，所有显式 `fallback_visit()` 调用已归零，`_vm_assign_to_target()` CPS generator helper 完全替代旧递归穿透路径。VM 主路径已真正脱离 Python 递归栈依赖（除 `IbUserFunction.call()` 内部参数绑定/scope push 部分仍使用同步调用外）。

剩余暂缓项：**C11**（node_protection 侧表重构）是最大尚未完成的结构性债务，触及编译器+序列化+运行时三层；**C5**（CSE 类删除）等待测试契约调整后处理。
...
**前提**：M3c ✅ + M3d-prep ✅（CPS handler 覆盖已扩展到 37 个节点类型，详见 `docs/COMPLETED.md §十五`）

**剩余待 CPS 化的节点（M3d 范围）**：
* 控制流：`IbFor`（含 llmexcept loop_resume 集成）、`IbTry`、`IbExceptHandler`、`IbRetry`、`IbCase`
* 表达式：`IbLambdaExpr`（含闭包捕获）、`IbBehaviorExpr`、`IbBehaviorInstance`
* 其他：`IbTypeAnnotatedExpr`、`IbIntentInfo`

**文件级修改清单**：

| 文件 | 改动性质 |
|------|---------|
| `core/runtime/vm/handlers.py` | 新增剩余节点的 CPS handler |
...
| `core/runtime/objects/kernel.py:822` | `IbUserFunction.call()` CPS 化（不再 try/except `ReturnException`），改用 Signal 帧拦截 |
| `core/runtime/exceptions.py` | 删除 `ReturnException`/`BreakException`/`ContinueException` 的实际使用 |
| `core/runtime/vm/task.py::ControlSignalException` | 删除（DEFERRED_CLEANUP C5） |

**出口契约（完成后）**：  
- 当前 926 个测试全部由新 `VMExecutor` 路径驱动通过  
