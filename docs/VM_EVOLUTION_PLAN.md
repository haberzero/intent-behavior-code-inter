...
> **基准状态**（2026-04-29）：**996 个测试通过**；Step 1–8 全部完成；**M1 + M2 + M3a + M3b + M3c + M3d + M4 + M5a + M5b + M5c + M6 已完成**（fn 新语法 + IbCell GC 根集合 + CPS 调度循环骨架 + 控制信号数据化 + llmexcept 调度化 + 主执行路径切换至 VMExecutor + 多 Interpreter 并发 + DDG 编译期分析 + LLMScheduler + dispatch-before-use + VM 规范 + 合规测试套件）；**fn declaration-side 语法已完成**；**URGENT_ISSUES 中等优先级全部清理**；**Phase 1 C6/C12 轻量债务已清理**（Signal→CSE→ReturnException 三层桥已消除；ScopeImpl 私有字段访问已封装）。  
...
```
已完成（Step 1–8 + M1 + M2 + M3a–M3d + M4 + M5a–M5c + M6 + Phase 1 C6/C12 轻量清理）
  └─── 当前状态（基准：996 tests）
         │
         ...
         └─── ✅ M5c：dispatch-before-use                             [已完成 2026-04-29]
                ...
                └─── ✅ M6：可移植性参考实现 + 合规测试套件（Step 12） [已完成 2026-04-29]
                       │
                       └─── ⏳ 编译器深度清洁（C7/C8/C9/C11/C14）     [下一主线 —— 依赖 M6 安全网]
```

**当前优先路径**：M6 完成后，以 `tests/compliance/` 合规测试套件作为安全网，推进 `docs/DEFERRED_CLEANUP.md` 中 C7/C8/C9/C11/C14 编译器级深度清洁任务（C8 最重要：让 IbLambdaExpr/IbBehaviorInstance 进入完整 CPS 路径，彻底兑现"消除 Python 递归依赖"的设计目标）。
