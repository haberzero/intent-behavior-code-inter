...
- `IbDeferred`/`IbBehavior` 的自由变量通过 `captured_scope` 直接持有整个 `ScopeImpl` 引用  
  → 公理 SC-3/SC-4（`IbCell` 独立堆对象）**未实现**（`builtins.py:710`）
- `fn`/`lambda`/`snapshot` 的现有语法不支持参数传递（`declaration.py:106–138`）
...
已完成（Step 1–8）
  └─── 当前状态（基准：678 tests）
         ...
         │
         ├─── M3a：CPS 调度循环骨架（Step 9a）                      [独立入口，M1 完成后更安全]
         │      │
...
**推荐执行顺序**：M1 → M3a → M3b → M3c → M2 → M4 → M5 → M6  
...
**目标**：IbCell 机制落地公理 SC-3/SC-4/LT-2/LT-3；更新 GC 根集合扫描路径。
...
| 文件 | 改动性质 |
|------|---------|
| `core/runtime/objects/cell.py` | 添加 `trace_refs()` 方法（供 GC 遍历）；添加弱引用支持 |
| `core/runtime/interpreter/runtime_context.py` | 新增 `collect_gc_roots()` 方法：返回全局作用域 ∪ 活跃帧局部变量 ∪ 所有活跃 fn.closure Cell 值 |
| `core/runtime/objects/kernel.py:721–734` | `IbUserFunction` 添加 `closure: Dict[str, IbCell]` 字段；`IbLLMFunction` 同步 |
| `core/runtime/objects/builtins.py` | `IbDeferred`/`IbBehavior` 的 `call()` 通过 `closure` 字典 deref Cell，不再使用 `captured_scope` |
| `core/runtime/objects/builtins.py` | 删除 `_captured_scope` 字段（旧机制） |

**出口契约（完成后）**：  
- lambda 闭包的自由变量通过 `IbCell` 间接访问（不再直接持有 ScopeImpl 引用）  
