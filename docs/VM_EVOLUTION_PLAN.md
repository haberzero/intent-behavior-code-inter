...
> **剩余技术债**：**无**——`docs/DEFERRED_CLEANUP.md` 中所有条目已 ✅ DONE。`ControlSignalException` 类本体作为兼容别名仍保留，但所有 production 路径均不再产生该异常。  
...
```
python3 -m pytest tests/ -q --tb=short   # 989 passed（2026-04-29，编译器深度清洁 Phase 1–5 完成后；C11/P3 删除 3 个 TestProtectionRedirect 死代码测试，由 992 → 989）
```
...
剩余暂缓项：**无**——`DEFERRED_CLEANUP.md` 已清零。`ControlSignalException` 类本体仅作为测试兼容别名保留，可在未来非破坏性 PR 中迁移到 `UnhandledSignal` 后删除。
...
| `core/runtime/vm/task.py::ControlSignalException` | 类本体保留为兼容别名（C5 工程目标已随 C11/P3 达成） |
