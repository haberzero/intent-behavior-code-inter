...
> 路径统一（ADR-015/017/018/019）已全部落地（2026-07-13，PT-ARCH-21 完成）。当前活跃主线为**变量存储模型 + 内核原生边界 + 类型化 flag 轴**（ADR-016/020/021 + ADR-013/014，即 PT-ARCH-23 里程碑）。这些 ADR 是 media Phase 4 的强制前置 gate，**非** media 决策本身。
...
- [ADR-020: 内核原生 vs 插件边界重划 + FileHandle 磁盘型基类](ADR-020-kernel-native-vs-plugin-boundary.md) — **设计阶段（2026-07-13，实现未开始）**：经 8-subagent 交叉验证。① 内核原生模块 = import-gated（非免 import；可用性 vs 可见性两轴正交）；② FileHandle = 磁盘型基类（axiom kernel / 值类 runtime / bootstrap 绑定三角色），media 为子类，ibci_file 消亡；③ bootstrap 针对性升级（懒查找 + loader 短路预注册）；④ 注册协议双路径保留（类型→axiom / 模块→vtable）；⑤ "builtin" 一词五义，删除为独立名词。协同 ADR-016/014；衍生命名清理 PT-ARCH-22。
