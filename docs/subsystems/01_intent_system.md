...
意图注释系统的合法语法形式全集（**语法权威**：`docs/syntax/09_intent_system.md` §9.1）。此处仅列实现侧关注的语义类别：
...
│   └── 语义上绑定下一条语句执行窗口（窗口内可出现普通调用或 LLM 调用）
...
| | 自由变量（普通） | 意图上下文 |
|---|---|---|
| **snapshot** | 定义时深克隆冻结，调用时再克隆为私有副本，无结果缓存 | 定义时 `fork()`，绝对免疫调用处意图 |
