...
Intent 类型全栈公理化：新增 `IntentAxiom`（`is_class()=True`，完整 vtable 声明）；`IbIntent` 添加 `@register_ib_type("Intent")` 并实现 `get_content()`/`get_tag()`/`get_mode()`；`INTENT_SPEC = ClassSpec(name="Intent")` 注册至默认 spec registry；`builtin_initializer.py` 确保 `@register_ib_type` 在公理绑定前执行。  
*文件：`core/kernel/axioms/intent.py`、`core/kernel/spec/specs.py`、`core/kernel/spec/registry.py`、`core/runtime/bootstrap/builtin_initializer.py`*

### 4.19 插件懒加载（显式引入基础实现）

`Engine.__init__()` 不再调用 `discover_all()`；改由 `Engine._ensure_plugins_discovered()` 实现懒加载，首次 `compile()` / `check()` 调用时执行一次插件发现。初始化阶段只创建空 `HostInterface`，不触发任何插件扫描。  
*文件：`core/engine.py`（`_ensure_plugins_discovered`）*
...
### 9.3 `DeferredAxiom.is_compatible` 语义矛盾修复
...
**背景**：落地 CPS 调度循环骨架——`Interpreter.visit()` 的并行路径，为 M3b/M3c/M3d 的控制流数据化、LLM exception 帧调度、主路径切换奠定调度层基础。
...
## 十二、DDG 编译期分析（BehaviorDependencyAnalyzer）
...
## 十三、IbLLMExceptionalStmt CPS 调度化
...
## 十六、VM 主路径切换 + LLM dispatch-before-use
...
### 19.4 轻量债务清理（Signal→CSE→ReturnException 三层桥消除）
...
### 19.5 轻量债务清理（ScopeImpl 私有字段访问封装）
...
> 编译器深度清洁 Phase 1 已记录于 §十九。本节汇总后续清洁工作，覆盖所有剩余技术债条目；自此该文件全部 ✅ DONE 并已归档进本文件后删除。
...
## §二十二：fn/lambda/snapshot 类型系统重设计
