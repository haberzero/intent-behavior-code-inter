...
| `T06_class_identity` | 统一类身份模型回归 + 真实试用（Task1 S1-S4 根治验证 + T05 KI-1 核销） | 2026-08-14 | 20 用例 | **2026-09-07 run_batch 复跑 20/20 PASS**（子目录布局收集 + 目录型用例 root 修复） | `KERNEL_ISSUE-CROSSMOD-LLM-1`（**已核销 2026-08-14，T07 D3 重验**） |
...
| `T10_llm_callable` | 五大地基新特性：llm 可调用类全面（直接调用/装配/解析/`__intent__` 三层改写/`__retry__` 高阶化/run_batch 逐项参数化）+ 装配 dict 未知键可见性（M5，2026-09-07） | 2026-08-21 | 14 例（mock 层） | 10 PASS + 4 GUARD（mock 层确定性全绿） | `BOUNDARY-LLM-4`（新登记）；LLM 层回归待运行 |
...
| `T15_edge_malicious` | 恶意边界测试（起点清单 33 项逐条核对 + 自行扩展：容器==/意图窗口/装配键/参数/协议异常/retry/泛型/递归/retry 体内文件写禁/线程 LLM 等；未测项见下「恶意边界后续未测项」） | 2026-08-21 | 22 例 | 14 PASS + 7 GUARD + 1 LIMIT（无内核缺陷；fail-fast + 编译期守卫全生效；>4k token/多轮/批量并发真实 LLM 全 PASS） | 无；原待文档复核项（装配 dict 未知键静默忽略）**已裁定并实施（2026-09-07：LLM_ASSEMBLY_UNKNOWN_KEY 警告——见 T10 M5）** |
...
| 19 | overlay 与序列化/snapshot/retry 交互 | ✅ **已测（2026-09-07）**：T15-E-M31 ① overlay × snapshot（作用域内调用经覆层渲染 / 定义于作用域内调用于作用域外=原渲染——overlay 是作用域化运行时状态，不被 snapshot 捕获，与值深克隆捕获语义正交）② overlay × 序列化（with 块内 save/load 后渲染回落原形态——overlay 状态不跨序列化边界，临时作用域状态非持久值）；③ overlay × retry 经机制同构确立（retry 复用首轮已渲染字符串不重新渲染——overlay 仅作用于首轮 dispatch 渲染，无附加对抗面）——三交互面零缺陷 | `T15/.../T15-E-M31-overlay-interactions.ibci` |
...
| `BOUNDARY-LLM-5` | 进程内 mock（ai.set_mock_mode）下 `get_current_call_info()` 无 `sys_prompt` 键（真实模式有） | **已登记（2026-08-21，T13）**：观测设施缺口（意图三层 intents 始终可用，T13 已改用其断言）；供阶段 C 文档复核评估观测 API 对 mock 的覆盖。**补充实证（2026-09-05，T08 复跑）**：真实模式下 dispatch-before-use 的 future 若从未被语言层消费，单写槽停留在 dispatch 快照形态（同样无 `sys_prompt`，response 为空）——resolve 点覆盖只在消费时发生；用例先消费再读即可观测完整 call_info（D3-09/D3-10 已修用例消费顺序）。**2026-09-07：call_info 观测契约（两形态快照：dispatch 请求面 → resolve 补全 response/sys_prompt/finish_reason/generation；变量读点触发 resolve）已文档化（docs/syntax/11_modules.md ai 节）——机制澄清完成，残留 = mock 路径 provider_meta 不含 generation 字段（mock 无真实采样姿态，观测面差异属预期）** | `T13/.../T13-I-M1-*.ibci` + `T08/.../D3-09/D3-10` |
...
| `KERNEL_ISSUE-IMPORT-2` | 模块 `import` 解析锚定 **project_root** 而非入口文件目录：多文件用例（main.ibci + 同目录模块）在 project_root ≠ 用例目录时 `DEP_MODULE_NOT_FOUND`；历史经"root=用例目录"的隐性耦合而通过，配置单源收敛后暴露 | **已修复（2026-09-07，C3 模块路径解析定案）**：绝对导入两级搜索（① 导入方文件目录同目录模块 → ② 项目根兜底）+ 模块 artifact 键 = 用户 import 名（运行期 import_module 查询名一致；包路径形态 = 完整包路径名）+ 合成入口载体 ① 级越界候选跳过（沙箱外不报错）；锚点语义文档化（docs/syntax/11_modules.md §11.1 模块路径解析）。判别测试 +5（tests/compiler/test_module_path_resolution.py：子目录同目录/根模块兜底/三段包路径/合成载体/跨模块特化键独立）；T06 全量复跑 20/20 PASS（harness "root=用例目录" 形态保持兼容） | `tests/compiler/test_module_path_resolution.py` + `T06/.../D1-02` |
...
| `KERNEL_ISSUE-STREAM-1` | `ai.stream_channel` 流句柄被放弃（只 recv 部分块即脚本结束）→ 消费 daemon 线程仍在 C 扩展内消费活 HTTP 流，解释器终结不等待 daemon → C 扩展状态释放竞态 **SIGSEGV**（退出阶段段错误，用例输出完整；2 并发 4/12 复现、单发 0/15，faulthandler 线程死在 C 扩展内） | **已修复（2026-09-06）**：流句柄生命周期闭环——`cancel()` 协作式截断（生成器单写者：CPython 禁跨线程 close 运行中生成器，消费线程 in-thread close 触发 provider finally 关 HTTP 流）+ producer 契约 = 生成器（fail-fast）+ atexit live 注册表 drain（有界 join，daemon 语义不变）；pytest +7 判别；并发复跑门 24/24 零段错误（修复前 4/12） | `T08/.../D5-02-stream-channel.ibci`（复现→并发回归门） |
| `KERNEL_ISSUE-SEM-1` | 比较运算符编译期类型检查缺失：静态已知失配的 `str >= int`（含 `(str)` cast 链、`str x = "a"; if x >= 0` 裸形态）不在语义分析期拦截，漏到运行期 `RUN_GENERIC_ERROR: TypeError: Cannot compare 'str' and 'int'`（无 ibci 源行、无诊断码定位）——与 B1/P2 同域（错误定位链缺口）；外部试用工程微测试（ibci-trial e32x p11_cast）复现，2026-09-06 本分支实测三形态全复现（str>=int 裸/cast 链/find 结果 cast） | **已修复（2026-09-07，诊断面打包 P0-1 批 4）**：公理层比较分支条件化（Int/Float/Bool/Str 排序比较 `< <= > >=` 仅数值族内/str↔str 合法；`== !=` 维持无条件——运行期跨型合法返回 False/True）+ `visit_IbCompare` 逐对检查（链式每对，any 放行 is_dynamic 惯例）+ `visit_IbBinOp` 兜底收紧（防误报前置：Int/Bool `* str` 声明字符串重复，float `* str` 不声明）。三形态转编译期 `SEM_TYPE_MISMATCH` @ ibci 源行列（`tests/compiler/semantic/test_semantic_type_checking.py` 23 项锁定，含三形态回归）；运行期残余面（any 放行值）经错误定位链（批 1/2/3）携带位置 + `RUN_TYPE_MISMATCH` 码 | `tests/compiler/semantic/test_semantic_type_checking.py`（原复现脚本 `.tmp_verify/p11*.ibci` 三形态转编译期拦截） |
| `KERNEL_ISSUE-SEM-2` | `in` 成员运算符编译期类型检查缺失（与 SEM-1 同面，分组裁定挂起）：`int in "abc"` 运行期才报（native `str.__contains__` 失败）；`1 in ["a"]` 运行期恒 False 合法——检查面 = 容器类型 × 元素类型矩阵 | **已登记（2026-09-07，P0-1 分组裁定）**：P0-1 不扩 `in` 检查（运行期错误面小且消息清晰；list 元素 `==` 恒合法需排除，误报面大）；后续与 P2 队列"dict 可 for 迭代"同批评估 | —（复现：`any a = 1 in "abc"` 运行期 ERR） |
| `KNOWN_LIMIT-PARSE-EOF` | 解析错误位置归位（P2，批 5）边界形态：未闭合括号 + 文件末尾（EOF 卡住点）——归位标记附于跨行生成 token，EOF 无宿主 token，错误仍报 EOF 行 | **已登记（2026-09-07，已知局限）**：低价值边界形态（报错消息 `Expect ')' ...` 已足够定位意图，EOF 行即文件末尾"缺失"语义）；不阻塞 P0-1 出口 | —（复现：`int n = (int` + EOF） |
| — | `KERNEL_ISSUE-VM-2` | fielded 用户类实例化 × LLM 调用场景 → `VM: missing required argument '<字段名>'`（试用方 R183 上报 D-R183-1，v3 持续复现；**归因修正 2026-09-08**：非 LLM/VM 分派污染——失败点在 fielded 类**零参构造器调用自身**（`class Ctx: str tag` + `Ctx c = Ctx()`：auto 构造器参数 = 链上无默认值字段，`Ctx()` 缺必填 = IBCI 合法报错）；真实缺陷 = 语义层无类构造器/方法调用静态参数绑定检查 → 编译通过、运行期裸 RuntimeError（无码/无 ibci 源行/无说明）误导试用方误读为内核 bug 并回避 fielded 类（数据建模硬阻塞） | **已修复（2026-09-08，round3 需求 D-1 / R3-①）**：① 语义层构造器调用静态绑定检查——`registry.get_constructor_descriptors`（explicit __init__ 精化描述符 / auto 构造器 = 链上有效无默认值字段，与运行期 hydration 共享 `merge_decl_fields` 单一规则源）接入统一 `_resolve_with_descriptors`（同一 resolve_call_binding + SEM_* 四码 + ibci 源行列 + kind 感知主语）；② 零参绑定方法 arity 补全（param_types = [] 收集期权威 → 多余实参结构裁决）；③ 运行期 auto-init 回退条件修正（链上无必填字段时区分：链上有字段全默认/子类覆盖 → 零参 auto-init；链上无字段 → 继承祖先显式构造器——原回退把两形态混为一谈，子类覆盖场景运行期误要求父类字段，编译期/运行期语义分裂）；判别测试 25 项（tests/compiler/semantic/test_constructor_call_binding.py）+ 既有 test_missing_arg_fails_fast 语义演进（运行期 → 编译期，触发场景保留）；试用方 min_repro 场景现编译期 `SEM_MISSING_REQUIRED_ARG` 拦截（带源行列）。残余面（登记）：动态接收者（any 类型 callee）的构造器/方法调用仍走运行期裁决——既有错误定位链残余，不扩本轮 | 复现资产 = 试用方 min_repro_defect.ibci / min_repro_control.ibci（ibci-trial e33_p24 目录，只读）+ tests/compiler/semantic/test_constructor_call_binding.py |
