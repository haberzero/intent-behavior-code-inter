...
| — | `KERNEL_ISSUE-VM-2` | fielded 用户类实例化（带 field 声明）× 任意 LLM 调用（`@~...~`/run_batch，可同函数可跨函数）→ `VM: missing required argument`（报错**参数名空**，与字段数/类名无关）——试用方数据建模（typed record）硬阻塞；试用方 R183 上报（D-R183-1），v3（R185–R202）持续复现 | **已登记（2026-09-08，round3 需求 D-1；流程缺口补登记——此前未入本台账）**：错误源 `core/runtime/vm/handlers/_shared.py:179`（callee param spec 参数名丢失的绑定路径）；复现 = mock 确定性形态（fielded 类实例化 + @~ mock 调用，免网络）；队列 = round3 整合 P0 首位（R3-①） | 复现资产 = 试用方 e34 系列 run 存档（ibci-trial 只读）+ 待建 mock 最小复现 |
