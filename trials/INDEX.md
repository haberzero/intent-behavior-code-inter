...
| — | `KERNEL_ISSUE-VM-2` | fielded 用户类实例化 × LLM 调用场景 → `VM: missing required argument '<字段名>'`（试用方 R183 上报 D-R183-1，v3 持续复现；**归因修正 2026-09-08**：非 LLM/VM 分派污染——失败点在 fielded 类**零参构造器调用自身**（`class Ctx: str tag` + `Ctx c = Ctx()`：auto 构造器参数 = 链上无默认值字段，`Ctx()` 缺必填 = IBCI 合法报错）；真实缺陷 = 语义层无类构造器/方法调用静态参数绑定检查 → 编译通过、运行期裸 RuntimeError（无码/无 ibci 源行/无说明）误导试用方误读为内核 bug 并回避 fielded 类（数据建模硬阻塞） | **根因已定（2026-09-08，round3 需求 D-1；流程缺口补登记——此前未入本台账）**：试用方 min_repro 独立复现 100%（8 行 mock 脚本，ibci-trial `experiments/e33_p24_long_session_stress/min_repro_defect.ibci` 只读存档）；盲区实证（缺必填/多参/未知具名构造器调用全部编译通过）；修复设计 = 语义层暴露构造器/方法 `param_descriptors` 接入统一绑定检查（同一 resolve_call_binding + SEM_* 四码 + 源行列；auto 构造器签名与运行期 hydration 规则同源）；队列 = round3 整合 P0 首位（R3-①，实施中） | 复现资产 = 试用方 min_repro_defect.ibci / min_repro_control.ibci（e33_p24 目录）+ 本侧盲区验证脚本（待入判别测试） |
