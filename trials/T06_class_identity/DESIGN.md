# T06_class_identity — 统一类身份模型回归 + 真实试用（2026-08-14）

> 2026-08-14。**试用与记录任务，不是修复任务**。目标：对统一类身份模型根治
> （`_code_class_identity_unify.md` S1-S4：入口类 qualified / 单类表 / KI-1 线程
> 侧表 / is_truthy 任务本地化）后的最新内核做**回归 + 对抗 + 真实 LLM** 全方位
> 试用，确认 Task1 宏观重构的健康与边界，并核销 T05 KI-1 触发用例。
>
> 基线：unsafe-vibe-dev（Task1 后），全量 2616 passed / 1 skipped。
>
> 关联：`_HANDOFF_T05_ISSUES.md`（KI-1 已根治，本套核销触发用例）。
...
| 用例 | 目标 |
|------|------|
| D4-01 | T05 D1-10 核销：线程 worker 内 geo.Box(5).get() = 405（KI-1 已根治） |
