...
> **规范（2026-08-13 迁移）**：本套现位于 `trials/T01_llm_full/`；分类/级别/编号
...
> 历史编号 → 统一命名空间。全部已修复（2026-08-12，PT-DEBT-25~28 / 文档批次），详见 PENDING_TASKS。

| 旧编号 | 新编号 | 主题 |
|--------|--------|------|
| KERNEL-ISSUE-001 | `KERNEL_ISSUE-VM-1` | global 写访问失效（PT-DEBT-25，已修） |
| KERNEL-ISSUE-002 | `KERNEL_ISSUE-IMPORT-1` | 整模块 import+成员访问 INT_INTERNAL_ERROR（PT-DEBT-26，已修） |
| KERNEL-ISSUE-003 | `KERNEL_ISSUE-LLM-1` | LLM provider 失败异常逃逸 try/except（PT-DEBT-27，已修） |
| KERNEL-ISSUE-004 | `KERNEL_ISSUE-CONFIG-1` | ai.get_retry 等 vtable 未注册（PT-DEBT-28，已修） |
| BOUNDARY-001 | `BOUNDARY-SEQ-1` | for=to_list 物化（文档化边界） |
| BOUNDARY-002 | `BOUNDARY-ASYNC-1` | 生成器 await chan 未复现 KNOWN_LIMITS §二十四 |
| BOUNDARY-003 | `BOUNDARY-CONFIG-1` | 隔离子环境不继承 LLM 配置 |
| BOUNDARY-004 | `BOUNDARY-LLM-1` | @! 在 run_batch 仅首调用生效 |
| BOUNDARY-005 | `BOUNDARY-CONFIG-2` | probe_model 误判推理模型 |
| DOC-ISSUE-001~007 | `DOC-ISSUE-001~007`（保持） | 文档批次已处置（commit 6f8506d） |
