...
> 全部经死循环保护 harness（进程组 SIGKILL + timeout）；30 次运行，唯一
...
| 旧编号 | 新编号 | 状态 |
|--------|--------|------|
| KERNEL-ISSUE-G1（方法体类型参数） | `KERNEL_ISSUE-GEN-1` | 已修复（32484fe） |
| KERNEL-ISSUE-G2（自引用字段） | `KERNEL_ISSUE-GEN-2` | 已修复（32484fe） |
| 双通道 descriptors | `KERNEL_ISSUE-GEN-3` | 已修复（32484fe） |
| BOUNDARY-G1（非法特化实参） | `BOUNDARY-GEN-1` | 已修复（3fe98d6） |
