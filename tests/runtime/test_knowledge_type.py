...
class TestSaveLoadFidelity:
    """save_state/load_state 保真（同入口程序形态——T15-E-M28 契约同型）。

    断言面 = 程序内行为（VM 读路径）：load 后 VM 变量读经 UID 绑定表
    （恢复快照），与测试侧直接访问作用域名称表不同面——与 T15-E-M28
    回归门同契约（程序内断言）。
    """

    def test_state_roundtrip_same_file(self, engine, tmp_path):
    ...
    def test_amend_after_restore_fail_fast(self, engine, tmp_path):
