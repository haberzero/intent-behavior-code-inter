...
REGISTERED: List[DeclaredState] = [
    ...
    DeclaredState(
        ...
        rationale=(
            ...
            "评估裁决（R2 续项）：维持 i64 有界契约（i128/任意精度 = JSON/通道/"
            "差分全局复杂度远超边际收益；大整数非 DSL 场景）；溢出 = 显式错误优于"
            "静默错误值；可扩展 = 未来独立 BigInt 值类型不改 Int 契约（见 "
            "docs/KNOWN_LIMITS.md §二十八）。语料无大整数探针，零现有偏离。"
        ),
    ),
]
