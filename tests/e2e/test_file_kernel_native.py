"""
tests/e2e/test_file_kernel_native.py
========================================

e2e 测试：fs 模块内核原生化 + 安全闸门。

覆盖：
1. import gate：`import fs` 把 file_handle/audio/image/video 类型注入作用域。
2. write 基本行为。
...
# 文件内核测试专用前缀（含 `import fs`；独立于 conftest 的 AI_MOCK_PREFIX，
# 避免同名遮蔽）。
...
class TestFileKernelNative:
    """fs 模块作为 kernel-native 模块的核心契约。"""

    def test_import_file_gates_file_handle_type(self, tmp_path):
        """`import fs` 后 file_handle 类型可直接使用。"""
        code = (
            'import fs\n'
            'file_handle fh = fs.open("./x.txt")\n'
            'print("ok")\n'
        )
    ...
    def test_import_file_gates_media_types(self, tmp_path):
        """`import fs` 后 audio/image/video 类型可直接使用。"""
    ...
    def test_media_type_without_import_fails(self, tmp_path):
        """未 import fs 时 audio 类型不可见，编译期 SEM_UNDEFINED_SYMBOL。"""
    ...
    def test_write_new_mode_leaves_original_untouched(self, tmp_path):
        ...
        code = (
            'import fs\n'
            'fs.write("./base.txt", "original", overwrite_flag="overwrite")\n'
            'file_handle h = fs.open("./base.txt")\n'
            'file_handle c = fs.write("./copy.txt", "copied")\n'
            'print(fs.read(h))\n'
            'print(fs.read(c))\n'
        )
        lines = run_ibci(code, root_dir=str(tmp_path))
    ...
    def test_write_overwrite_mode_mutates_shared_backing(self, tmp_path):
        ...
        code = (
            'import fs\n'
            'fs.write("./shared.txt", "before", overwrite_flag="overwrite")\n'
            'file_handle a = fs.open("./shared.txt")\n'
            'file_handle b = fs.open("./shared.txt")\n'
            'fs.write(a, "after", overwrite_flag="overwrite")\n'
            'print(fs.read(b))\n'
        )
        lines = run_ibci(code, root_dir=str(tmp_path))
    ...
    def test_write_new_mode_creates_file(self, tmp_path):
        ...
        code = (
            'import fs\n'
            'file_handle h = fs.write("./new.txt", "created")\n'
            'print(fs.read(h))\n'
            'print(h.path)\n'
        )
        lines = run_ibci(code, root_dir=str(tmp_path))
    ...
    def test_sandbox_rejects_path_outside_project_root(self, tmp_path):
        ...
        expect_runtime_error(
            'import fs\nfs.write("../outside.txt", "x", overwrite_flag="overwrite")\n',
            "Security Error",
...
class TestFileSecurityGates:
    ...
    def test_save_state_rejects_active_file_handle(self, tmp_path):
        ...
        expect_runtime_error(
            'import fs\nimport ihost\n'
            'fs.write("./x.txt", "x", overwrite_flag="overwrite")\n'
            'file_handle h = fs.open("./x.txt")\n'
    ...
    def test_llmexcept_retry_body_runtime_guard_catches_dynamic_dispatch(self, tmp_path):
        ...
        code = FILE_MOCK_PREFIX + (
            'func _do_write() -> auto:\n'
            '    fs.write("./x.txt", "mutated", overwrite_flag="overwrite")\n'
            'fn f = _do_write\n'
            'fs.write("./x.txt", "initial", overwrite_flag="overwrite")\n'
            'try:\n'
        ...
        expect_runtime_error(
            code,
            "fs.write is disabled inside an llmexcept retry body",
            root_dir=str(tmp_path),
        )
