...
- **R-F 验收面（对拍 + 确定性）**：`to_ibci()` 导出的投影代码经**独立 CLI run**
  执行重建等价 KB → 查询结果数据面**逐字节一致**（两次独立 run）+ 零 LLM 凭证
  （`deterministic.llm_calls == 0`）+ exit ok（投影 = 纯代码派生视图，零 LLM）；
...
class TestProjectionViaCli:
    def test_projection_deterministic_zero_llm(self, tmp_path):
        """R-F 确定性：投影代码两次独立 CLI run 数据面逐字节一致 + 零 LLM。"""
        ...
        assert results[0][0] == "True"      # atom composed_of proton
        assert results[0][1] == "True"      # atom composed_of electron
        assert "proton" in results[0][2] and "electron" in results[0][2]
        # 两次独立 run 逐字节一致（确定性派生视图）
