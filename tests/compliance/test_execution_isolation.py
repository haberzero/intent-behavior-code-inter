...
class TestCollectTimeout:
    ...
    def test_timeout_raises_when_child_exceeds_deadline(self):
        ...
        子任务先 sleep（0.5s）再返回，配合极小 collect_timeout（1ms），
        保证超时必然触发——不依赖"引擎启动耗时 > 超时"的脆弱墙钟假设
        （F3 移除插件发现后子引擎启动更快，原 1ms 假设曾偶发失效）。
        超时后子线程作为 daemon 孤儿继续运行；短暂等待让其自行结束，
        避免残留线程读取已被 finally 删除的临时文件。
        """
        child = write_child(
            "import time\n"
            "time.sleep(0.5)\n"
