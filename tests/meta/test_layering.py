"""
tests/meta/test_layering.py — 新测试体系分层红线（静态扫描）。
...
def _find_test_files(subdir: str):
    return sorted((TESTS_ROOT / subdir).glob("test_*.py"))
...
class TestKernelLayer:
    ...
    def test_kernel_does_not_use_engine(self, test_file):
        ...
        assert "run_ibci(" not in src and "from tests.conftest import run_ibci" not in src, (
    ...
    def test_e2e_does_not_import_runtime_internals(self, test_file):
        ...
        for pattern in self.BANNED_IMPORTS:
            assert pattern not in src, (
                ...
                f"— these are interpreter/VM internals. Move the test to tests/runtime/."
            )
