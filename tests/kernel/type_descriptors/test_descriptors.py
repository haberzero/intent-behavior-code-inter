...
class TestTypeDescriptor(unittest.TestCase):
    ...
    def test_type_descriptor_clone_preserves_members(self):
        """测试克隆保留成员"""
        from core.kernel.symbols import Symbol, SymbolKind
        desc = TypeDescriptor(name="test")
        ...
        self.assertIn("x", cloned.members)
        self.assertEqual(cloned.members["x"].name, "x")
        self.assertEqual(cloned.members["x"].kind, SymbolKind.VARIABLE)
