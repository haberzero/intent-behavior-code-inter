...
**读者定位红线（强制）**：`docs/` 定位为**人类手册**（指导手册 / 技术手册 / 参考手册），**绝对不允许存在任何关于本工作智能体相关的元信息**（工作流 / 过程 / agent 指令 / skill 引用 / opencode 配置）。此类信息只应存在于 `AGENTS.md` 与 `.opencode/skills/` 工作流层。治理 `docs/` 时发现任何此类信息一律**移除**（不改写为人类可读表述——从原则上它就不应存在）。
