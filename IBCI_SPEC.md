...
使用 `lambda` 或 `snapshot` 关键字声明一个**延迟执行的行为对象**，而不是立即触发 LLM 调用。声明左侧使用 `fn` 作为推导哨兵；返回类型标注（可选）写在表达式侧 `-> TYPE`：
...
- 表达式侧 `-> TYPE` 同时是该行为对象的**返回值类型标注**和注入给 LLM 的输出格式约束（D2，2026-04-29）
- 省略 `-> TYPE` 时返回类型由表达式右侧推导
- **声明侧形式 `TYPE fn NAME = lambda: EXPR`（如 `int lambda compute = ...`）已废弃**，会产生 PAR_003 编译错误（D1，2026-04-29）
...
- **数据路径**（`file.read` / `ihost.run_isolated` 等）：所有相对路径都基于**入口文件目录**解析，无论在哪个 IBCI 文件中执行。经规范解析器 `PathResolver`（entry_dir 单锚点）统一处理。
- **模块导入路径**（`import .foo` / `import ..bar`）：相对导入基于**导入者文件所在目录**解析（与 Python 相对导入语义一致），而非入口目录。

> 历史版本曾笼统声称"所有相对路径都基于入口文件目录解析"——这对模块导入是不准确的；已据实修正（2026-06-25 路径系统统一化，ADR-015）。
